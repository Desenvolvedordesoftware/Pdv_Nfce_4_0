...
begin
  ...
  begin
   ...
   can_venda.Close;
   can_venda.sql.clear;
   can_venda.sql.add('delete from c000048 ');
   can_venda.sql.add('where NUMERO_CUPOM_FISCAL LIKE ''%' +
    grid_cupom.Cell[4,grid_cupom.selectedrow].asstring+ '%''');
   can_venda.execsql;

  can_venda.Close;
  can_venda.sql.clear;
  can_venda.sql.add('delete from c000044');
  can_venda.sql.add('where HISTORICO LIKE ''%' +
    grid_cupom.Cell[4,grid_cupom.selectedrow].asstring+ '%''');
    query.execsql;

  can_venda.Close;
  can_venda.sql.clear;
  can_venda.sql.add('delete from c000124 ');
  can_venda.sql.add('where COD_VENDA LIKE ''%' +
    grid_cupom.Cell[4,grid_cupom.selectedrow].asstring+ '%''');
    query.execsql;
  ...
  can_venda.Close;
  can_venda.sql.clear;
  can_venda.sql.add('delete from c000032 ');
  can_venda.sql.add('where NUMERONOTA LIKE ''%' +
    grid_cupom.Cell[4,grid_cupom.selectedrow].asstring+ '%''');
    query.execsql;
...
begin
  ...
  UniQuery1.close;
      UniQuery1.sql.clear;
      UniQuery1.sql.add('select CUPOM.*, CUPOM_FORMA.*, VENDEDORES.*');
      UniQuery1.sql.add('FROM CUPOM, CUPOM_FORMA, VENDEDORES');
      UniQuery1.sql.add('where ');
      UniQuery1.sql.add('cupom.codigo = CUPOM_FORMA.cod_cupom and');
      UniQuery1.sql.add('cupom.COD_VENDEDOR = VENDEDORES.CODIGO');
