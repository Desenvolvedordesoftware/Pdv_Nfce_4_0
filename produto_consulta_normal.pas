...
uses
  ...
  NxCustomGrid, NxGrid, DB, DBAccess, Menus, AdvMenus, ComCtrls, Uni, MemDS,
  Vcl.Grids, vcl.wwdbigrd, vcl.wwdbgrid, frxClass, frxDBSet, AdvGlowButton;
...
type
  ...
  //  query: TIBCQuery;
    ...
    Frx_Impressaopedido: TfrxReport;
    frxDBPedido: TfrxDBDataset;
    qrimprimir: TUniQuery;
...
begin
   ...
   qrpedido.close;
   qrpedido.sql.clear;
   qrpedido.SQL.Add('select DESCRICAO, QTDE ');
   qrpedido.SQL.Add('from pedido_compra');
   qrpedido.open;
...
procedure TfrmProduto_consulta_normal.ImprimirClick(Sender: TObject);
begin
                      begin
                        BlockInput(false);
                        if application.messagebox
                          (pwidechar('Imprimir Pedido de Compra?' + #13 ), 'Aviso', mb_yesno) = idno then
                          exit;
           ...
           qrimprimir.close;
           qrimprimir.sql.clear;
           qrimprimir.SQL.Add('select DESCRICAO, QTDE');
           qrimprimir.SQL.Add('from pedido_compra');
           qrimprimir.ExecSQL;
          //query.open;

         Frx_Impressaopedido.ShowReport();
end;
