...
//  frmMsg_Operador.Refresh;
  ...
  qrPre_Venda.SQL.ADD('  c000049.codvenda, c000049.codcaixa, c000049.codvendedor,');
  qrPre_Venda.SQL.ADD('  c000049.data_emissao, c000049.codcliente, c000049.valor_original,');
  qrPre_Venda.SQL.ADD('  c000049.valor_desconto,c000049.valor_juros,');
  qrPre_Venda.SQL.ADD('  c000007.Nome Cliente,');
  qrPre_Venda.SQL.ADD('  c000008.Nome Vendedor');
  qrPre_Venda.SQL.ADD('from');
  qrPre_Venda.SQL.ADD('  c000049, c000007, c000008');
  qrPre_Venda.SQL.ADD('where');
  qrPre_Venda.SQL.ADD('  c000049.codcliente = c000007.codigo and');
  qrPre_Venda.SQL.ADD('  c000049.codvendedor = c000008.codigo');
  //qrPre_Venda.SQL.ADD('  c000049.tipo <> 9 and');
  //qrPre_Venda.sql.add('  c000074.situacao = 1');
  qrPre_Venda.sql.add('ORDER BY c000049.codvenda');
  qrPre_Venda.OPEN;
  ...
  begin
    ...
    begin
      if grid_venda.cell[0,q].asstring = qrPre_Venda.fieldbyname('codvenda').asstring then
      begin
        b := true;
        break;
      end;
    ...
    begin
      ...
      grid_venda.Cell[0,i].Asstring := qrPre_Venda.fieldbyname('codvenda').asstring;
      grid_venda.Cell[1,i].AsDateTime := qrpre_venda.fieldbyname('data_emissao').asdatetime;
      grid_venda.Cell[2,i].Asstring := qrPre_Venda.fieldbyname('cliente').asstring;
      grid_venda.Cell[3,i].Asfloat := qrPre_Venda.fieldbyname('valor_original').asfloat;
      ...
      grid_venda.Cell[7,i].Asfloat := qrPre_Venda.fieldbyname('valor_desconto').asfloat;
      grid_venda.Cell[8,i].Asfloat := qrPre_Venda.fieldbyname('valor_juros').asfloat;
    end;
    qrpre_venda.Next;
  end;
...
begin
  ...
  begin
    if Application.messagebox('Não a itens lançados para esta venda! Deseja fazer o cancelamento?','Atenção!',mb_yesno+MB_ICONEXCLAMATION) = idno then
      exit
    else
  ...
  begin
    if not bConcilia then
      if application.messagebox('Confirma a impressão do cupom desta venda?','Atenção',mb_yesno+
...
begin
  ...
  begin
    ...
    if application.messagebox('Confirma o cancelamento desta venda?','Atenção',mb_yesno+
  ...
  i := grid_venda.SelectedRow;
      ...
      sPre_Venda_Numero := 'PV'+ZERAR(grid_venda.Cell[0,i].asSTRING,10);
          ...
          frmMsg_Operador.lb_msg.caption := 'Aguarde cancelando venda nº '+grid_venda.cell[0,i].asstring+'...';
          frmMsg_Operador.Show;
          frmMsg_Operador.Refresh;
...
begin
  try
    ...
    if x < 2 then
    begin
     Application.messagebox('Favor selecionar mais de uma venda!','Erro',MB_OK+MB_ICONERROR);
     exit;
    end;
