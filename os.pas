...
uses
  ...
  RzLabel, jpeg, ExtCtrls, Menus, AdvMenus, DB, DBAccess, pngimage, Uni, MemDS,
  AdvGlowButton, Vcl.Mask, JvExMask, JvToolEdit;
...
type
  ...
  //  QRITEM: TIBCQuery;
    ...
    ed_data: TJvDateEdit;
    Label1: TLabel;
    filtar_por_data: TAdvGlowButton;
    ...
    procedure filtar_por_dataClick(Sender: TObject);
  private
...
procedure TfrmOS.filtar_por_dataClick(Sender: TObject);
var i : integer;
x : integer;
sNumero_dav : string;
begin
...
//  frmMsg_Operador.Refresh;
  ...
  qrOS.SQL.CLEAR;
  qrOS.SQL.ADD('select');
  qrOS.SQL.ADD('  c000048.*,');
  qrOS.SQL.ADD('  c000007.Nome Cliente,');
  qrOS.SQL.ADD('  c000008.Nome Vendedor');
  qrOS.SQL.ADD('from');
  qrOS.SQL.ADD('  c000048, c000007, c000008');
  qrOS.SQL.ADD('where');
  qrOS.SQL.ADD('  c000048.data = :data and');
  qrOS.SQL.ADD('  c000048.codcliente = c000007.codigo and');
  qrOS.SQL.ADD('  c000048.codvendedor = c000008.codigo and');
  qrOS.SQL.ADD('c000048.SITUACAO = 1 and c000048.GERADO_NFCE = ''N''');
  qrOS.sql.add('ORDER BY c000048.CODIGO');
  qrOS.parambyname('data').asdatetime := ed_data.Date;
  qrOS.OPEN;
  ...
  begin
    ...
    grid_venda.Cell[0,i].Asstring := qrOS.fieldbyname('NUMERO_CUPOM_FISCAL').asstring;
    ...
    grid_venda.Cell[7,i].Asstring := qrOS.fieldbyname('codvendedor').asstring;
...
begin
  ...
  if grid_venda.SelectedRow < 0 then
  begin
    application.MessageBox('Favor selecionar uma VENDA de serviço!','Atenção',MB_OK+MB_ICONWARNING);
    exit;
  end;
  ...
  frmmodulo.query_servidor.SQL.Add('select orc.CODIGO, orc.codproduto, orc.unitario, orc.total, orc.desconto,');
  frmmodulo.query_servidor.SQL.Add('orc.unidade,orc.qtde,orc.acrescimo, prod.produto, prod.codbarra, prod.cst, prod.aliquota');
  frmmodulo.query_servidor.sql.add('from c000032 orc,');
  frmmodulo.query_servidor.sql.add('c000025 prod where orc.codproduto = prod.codigo and orc.NUMERONOTA = '''+
                  grid_venda.cell[0,i].asstring+'''');
  frmmodulo.query_servidor.Open;


  QRITEM.SQL.CLEAR;
  QRITEM.SQL.ADD('select * from c000048');
  qritem.sql.add('where NUMERO_CUPOM_FISCAL = '''+qros.fieldbyname('codigo').asstring+'''');
  QRITEM.OPEN;

  frmmodulo.qrvendanfce.close;
  frmmodulo.qrvendanfce.SQL.CLEAR;
  frmmodulo.qrvendanfce.SQL.ADD('select * from c000048');
  frmmodulo.qrvendanfce.sql.add('where NUMERO_CUPOM_FISCAL = '''+grid_venda.cell[0,i].asstring+'''');
  frmmodulo.qrvendanfce.OPEN;

  frmvenda.lbcodvenda.Caption := grid_venda.cell[0,i].asstring;

  if (frmmodulo.query_servidor.RecordCount = 0) and
     (qritem.RecordCount = 0) then
  begin
    if Application.messagebox('Não itens ou serviços lançados para esta venda.! Deseja fazer o cancelamento?','Atenção!',mb_yesno+MB_ICONEXCLAMATION) = idno then
      exit
    else
    close // Cancelar1Click(frmOS);
  end
  else
  begin
    if application.messagebox('Confirma o NFC-e para esta venda.?','Atenção',mb_yesno+
...
procedure TfrmOS.CentralizarPanel(p:TPanel);
begin
...
procedure TfrmOS.FormResize(Sender: TObject);
begin
