...
uses
  ...
  Mask, RzEdit, RzPanel, DBAccess, AdvReflectionLabel, pngimage, AdvPicture,
  ...
  System.ImageList, SysUtils, AdvSmoothButton, Vcl.PlatformDefaultStyleActnCtrls,
  MidasLib, ACBrSocket, ACBrIBPTax, ACBrNCMs, JvExMask, JvToolEdit;
...
type
  ...
  TfrmPrincipal = class(TForm)
    ...
    lbl3: TLabel;
    label8: TLabel;
    advrflctnmg1: TAdvReflectionImage;
    ...
    procedure edataClick(Sender: TObject);
...
procedure AcertaPadraoData; External '\GCSISTEM\server\dll\ebo7.dll';

function NumeroSerieHD: string; External '\GCSISTEM\server\dll\ebo7.dll';
function frm(ebo: string): string; External '\GCSISTEM\server\dll\ebo7.dll';
...
uses modulo, senha, cliente,caixa_abertura,
  ...
  form_ativacaoicloud, pUCGeral, OpNatureza, Unit_ativar, ExportaXML, GridOrcamentos,
  ManutencaoProduto, AtualizaNCM, AtualizaIBPT, IcmsUF, PartilhaICMS, xloc_Pessoas;
...
//informaÁıes de ativaÁ„o do sistema
 ...
 //fim
  ...
  begin
    ...
    data := DataDeCriacao(extractfilepath(paramstr(0)) +'GCS_Gerencial.exe');
    ...
    lbl3.caption := frmmodulo.qrFilialFILIAL.asstring;
...
begin
  ...
  try
    ...
    ExeAtualiza := ExtractFilePath(ParamStr(0)) + 'atualizacao\GCS_Gerencial.exe';
    ExeAtual := ExtractFilePath(ParamStr(0)) + 'GCS_Gerencial.exe';
    ...
    begin
      ...
      begin
        if application.messagebox
          (Pchar('Foi Encontrada uma nova vers„o do Sistema SQL Gerencial 2.0.5.' + #13 +
        ...
        begin
          ...
          CopyFile(Pchar(ExeAtualiza),
            Pchar(extractfilepath(paramstr(0)) +'GCS_Gerencial.exe'), false);
          showmessage('AplicaÁ„o atualizada com sucesso!');
          Hand := FindWindow('TApplication', 'GCS_Gerencial');
          if Hand = 0 then
            frmPrincipal.ExecutePrograma(extractfilepath(paramstr(0)) +'GCS_Gerencial.exe', '');
          application.Terminate;
        end
        else
          novaversao := false;
...
begin
  WinExec('C:\GCSISTEM\ferramentas\suporte.exe', SW_SHOW);
end;
...
begin
  ...
  if (timetostr(time) >= '17:05:00') and (timetostr(time) <= '24:00:00') then
  begin
...
procedure TfrmPrincipal.AdvSmoothButton37Click(Sender: TObject);
begin
   barra_movimento.Visible:= False;
        frmxloc_Pessoas := Tfrmxloc_Pessoas.create(self);
        frmxloc_Pessoas.showmodal;
end;
...
begin
  ...
  frmPrincipal.ExecutePrograma('\GCSISTEM\server\sped.exe', '');
end;
...
procedure TfrmPrincipal.edataClick(Sender: TObject);
var
  vDate : TDate;
begin
  vDate := Date();
...
procedure TfrmPrincipal.edataEnter(Sender: TObject);
//var
//  vDate : TDate;
begin
//  vDate := Date();
...
procedure TfrmPrincipal.edataExit(Sender: TObject);
begin
tedit(sender).color := clwhite;
end;

procedure TfrmPrincipal.edataKeyPress(Sender: TObject; var Key: Char);
//var
//  vDate : TDate;
begin
  //vDate := Date(edata.date);
