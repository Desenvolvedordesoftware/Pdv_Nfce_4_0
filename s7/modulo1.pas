...
uses
  ...
  ZAbstractConnection, Data.DBXFirebird, Data.FMTBcd,
  Datasnap.DBClient, Datasnap.Provider, Data.SqlExpr, ACBrBoleto,
  //ACBrBoletoFCFR,
  ACBrBase, ZSqlProcessor, MemDS, Uni, ACBrBoletoFCFR; // NeoKeyUnit,frxExportImage,frxExportRTF,

type
  Tfrmmodulo1 = class(TDataModule)
...
var
  ...
  telaprincipal1: string; // variavel para mudar a tela principal
...
uses principal1; //produto,
//ecf;

{$R *.dfm}

procedure Tfrmmodulo1.titulo(parcel:integer;CODREC:string ) ;
...
procedure Tfrmmodulo1.DataModuleCreate(Sender: TObject);
var
  txt: textfile;
  entrada: string;
  ARQUIVO: string;
...
//  if ShortDateFormat <> 'dd/MM/yyyy' then
  ...
  caminho := '\GCSISTEM\server\ini\com.ini'; // Abre o Sistema Atual
  ...
  begin
    ...
    if copy(entrada, 1, 7) = '000-000' then
      telaprincipal1 := trim(copy(entrada, 9, 50));
    ...
    begin
      if trim(copy(entrada, 9, 50)) = 'LOCALHOST' then
        frmprincipal1.caption :=
          'SQL Gerencial 2.0.5'
      else
        frmprincipal1.caption :=
          'SQL Gerencial 2.0.5                 [ Servidor: ' +
    ...
    if copy(entrada, 1, 7) = 'ver-dad' then
      mostra_principal1 := copy(entrada, 9, 1);
...
procedure Tfrmmodulo1.qrLogBeforePost(DataSet: TDataSet);
begin
  qrLog.FieldValues['codigo'] := frmprincipal1.codifica('000002');
end;

procedure Tfrmmodulo1.qrsubgrupoBeforePost(DataSet: TDataSet);
begin
  ...
  if frmmodulo1.qrsubgrupo.state = dsinsert then
  begin
    frmmodulo1.qrsubgrupo.fieldbyname('codigo').asstring :=
      frmprincipal1.codifica('000018');
    frmmodulo1.qrsubgrupo.fieldbyname('codgrupo').asstring :=
      frmmodulo1.qrgrupo.fieldbyname('codigo').asstring;
  end;
...
procedure Tfrmmodulo1.Timer1Timer(Sender: TObject);
begin
...
procedure Tfrmmodulo1.qrgrade_subgrupoBeforePost(DataSet: TDataSet);
begin
  if frmmodulo1.qrgrade_subgrupo.state = dsinsert then
  begin
    frmmodulo1.qrgrade_subgrupo.fieldbyname('codigo').asstring :=
      frmprincipal1.codifica('000020');
    frmmodulo1.qrgrade_subgrupo.fieldbyname('codsubgrupo').asstring :=
      frmmodulo1.qrsubgrupo.fieldbyname('codigo').asstring;
  end;
end;

procedure Tfrmmodulo1.qrfiscal_cstAfterPost(DataSet: TDataSet);
begin
  Conexao.Commit;
end;

procedure Tfrmmodulo1.qrfiscal_cstAfterDelete(DataSet: TDataSet);
begin
  Conexao.Commit;
end;

procedure Tfrmmodulo1.qrservicos_periodicosCalcFields(DataSet: TDataSet);
begin
...
procedure Tfrmmodulo1.qrproduto_movBeforePost(DataSet: TDataSet);
begin
...
procedure Tfrmmodulo1.qrrelatorioAfterPost(DataSet: TDataSet);
begin
  ConexaoLocal.Commit;
end;

procedure Tfrmmodulo1.qrfiscal_cstBeforePost(DataSet: TDataSet);
begin
  qrfiscal_cst.fieldbyname('situacao').asstring :=
    ansiuppercase(qrfiscal_cst.fieldbyname('situacao').asstring);
end;

procedure Tfrmmodulo1.qrmarcaBeforePost(DataSet: TDataSet);
begin
  if CD_MAR = '1' then
  begin
    try
...
procedure Tfrmmodulo1.qrNF_CTEAfterInsert(DataSet: TDataSet);
begin
  frmmodulo1.qrNF_CTE.fieldbyname('codcte').asstring :=
    frmmodulo1.qrconhecimento.fieldbyname('numero').asstring;
  frmmodulo1.qrNF_CTE.fieldbyname('codigo').asstring :=
    frmprincipal1.codifica('NF_CTE');
end;

procedure Tfrmmodulo1.qrgrupoBeforePost(DataSet: TDataSet);
begin
  if CD_GRU = '1' then
  begin
    try
...
procedure Tfrmmodulo1.qrfornecedorBeforePost(DataSet: TDataSet);
var
...
procedure Tfrmmodulo1.qrclienteBeforePost(DataSet: TDataSet);
var
...
procedure Tfrmmodulo1.qradic_mestreAfterPost(DataSet: TDataSet);
begin
  conexao_adic.Commit;
end;

procedure Tfrmmodulo1.time_gpTimer(Sender: TObject);
begin
  // TEF_DIAL
  tempo_gp := tempo_gp + 1;
end;

procedure Tfrmmodulo1.qragendaBeforePost(DataSet: TDataSet);
begin
  qragenda.FieldValues['codigo'] := frmprincipal1.codifica('000034');
end;

procedure Tfrmmodulo1.Busca_Integridade_Base_Dados();
...
// checa se existe indice
  ...
  begin
    bAchou := false;
    QueryTmp := TZQuery.Create(Application);
    // QueryTmp.Transaction := frmmodulo1.TR;
    QueryTmp.Connection := Conexao;
    QueryTmp.sql.clear;
    QueryTmp.sql.add
...
procedure Tfrmmodulo1.timerKeyTimer(Sender: TObject);
var
  ret: Integer;
  status: string;
  sChallenge: string;
