...
uses
  ...
  ACBrNFeDANFEFRDM, ACBrNFeDANFEClass, ACBrNFeDANFEFR, ACBrEAD, Uni,
  UniProvider, InterBaseUniProvider, Datasnap.DBClient, MemDS, VirtualTable,
  ACBrValidador, AdvMenus, AdvMenuStylers, ACBrBase, ACBrBAL, pcnConversao,
  ACBrDFeSSL, blcksock, Vcl.FileCtrl, pcnConversaoNFe, ACBrUtil, System.Math,
  System.TypInfo, ACBrDFeReport, ACBrDFeDANFeReport, ExeInfo;
...
type
  ...
  TfrmModulo = class(TDataModule)
    ...
    Balanca: TACBrBAL;
    qrfatasia: TUniQuery;
    spNao_FiscalSV: TUniStoredProc;
    ExeInfo1: TExeInfo;
    qrvendanfce: TUniQuery;
    spCupom_FormaSV: TUniStoredProc;
...
uses funcoes, principal,ufrmStatus, venda;
...
//        PathSchemas        := Ini.ReadString('Geral','PathSchemas',PathWithDelim(ExtractFilePath(Application.ExeName))+'Schemas\'+GetEnumName(TypeInfo(TpcnVersaoDF), integer(Ini.ReadInteger( 'Geral','VersaoDF',0))));
...
procedure TfrmModulo.spCupom_Item_TempAfterExecute(Sender: TObject;
  Result: Boolean);
begin
...
procedure TfrmModulo.spCupom_Temp_deleteAfterExecute(Sender: TObject;
  Result: Boolean);
begin
  qrItens.DisableControls;
  qrItens.Close;
