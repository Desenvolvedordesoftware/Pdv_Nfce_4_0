...
begin
  ...
  begin
   { Imp.Porta := sPorta;
    ...
    Imp.Imprime(Repl('-', iLarguraBobina), [Centraliza]);
    Imp.Imprime('** Meios de Pagamento **', [Esquerda]);
    Imp.Imprime(CompletaStr('SUBTOTAL', '.', 20) + ': ' +
      FormatFloat(mascara_Valor, rTotal_venda), [Esquerda]);
    Imp.Imprime(CompletaStr(s + '(' + tipo + ')', '.', 20) + ': ' +
      FormatFloat(mascara_Valor, valor), [Esquerda]);
    Imp.Imprime(CompletaStr('TOTAL', '.', 20) + ': ' +
...
begin
  ...
  {  Imp.Porta := sPorta;
    Imp.Concomitante := True;
    Imp.Imprime(CompletaStr(forma_pgto, '.', 20) + ': ' +
      FormatFloat(mascara_Valor, valor), [Esquerda]);
...
begin
  ...
  begin
   { Imp.Porta := sPorta;
    ...
    Imp.Imprime(texto_justifica('Sistema - GCSistem_v2019', 19, ' ', taDireita), [t17cpi]);
...
begin
  ...
  begin
    ...
    {Imp.Imprime(CompletaStr(FormatFloat(mascara_qtde, quantidade) + ' X ' +
      unidade + '  X ', ' ', 9) + FormatFloat(mascara_Valor, valor_unitario) +
      '    =    ' + FormatFloat(mascara_Valor, total), [Esquerda]);
...
begin
  ...
  begin
    ...
    Imp.Concomitante := True;
    Imp.Imprime('', [Centraliza]);
    Imp.Imprime('*VENDA CANCELADA PARA ORCAMENTO* ', [Centraliza]);
    Imp.Imprime('', []);
    Imp.Imprime('', []);
    Imp.Imprime('', []);
    Imp.Imprime('', []);
...
begin
  ...
  begin
   { Imp.Porta := sPorta;
    Imp.Concomitante := True;
    Imp.Imprime(emitente_fantasia, [fNegrito, Esquerda]);
    Imp.Imprime(emitente_endereco, [Esquerda]);
    Imp.Imprime(emitente_cidade, [Esquerda]);
    Imp.Imprime('Telefone: ' + emitente_telefone, [Esquerda]);
    Imp.Imprime(Repl('-', 29), [Esquerda]);
    Imp.Imprime('No.' + Zerar(sNumero_Cupom, 6) + ' Emis:' +
      DateToStr(dData_Sistema) + ' ' + Copy(TimeToStr(Time), 1, 5) + ' Caixa:'
      + Zerar(sECF_Caixa, 4), [Centraliza]);
    Imp.Imprime(Repl('-', 29), [Esquerda]);
    Imp.Imprime('PROD. QTDE X VLR.UN = VLR.TOTAL',
      [Esquerda]);
    Imp.Imprime(Repl('-', 29), [Esquerda]);
...
begin
  try
    ...
    Result := IsFloat(frmPrincipal.adic_codifica('000048'));
