...
uses
  ...
  Vcl.ComCtrls, CPort, cxContainer, dxBarCode, dxSkinTheBezier;
...
type
  TfrmVenda = class(TForm)
    ...
    procedure dxBarCode_QrCode_pixClick(Sender: TObject);
...
uses modulo, Math, funcoes, cliente_consulta, preco_consulta,
  ...
  Senha_ADM, Bloqueio_estoque, Status, StatusNFC, pix;
...
function TfrmVenda.PayLoad(ChavePIX: String;Beneficiario: String;Cidade: String;CodTransferencia: String;Valor: Real): String;
...
begin
  try
    ...
    begin
      ...
      frmVenda.qrprodutos_vr.close;
      frmVenda.qrprodutos_vr.sql.clear;
      frmVenda.qrprodutos_vr.sql.add('select * from C000025 pro, c000100 est');
      frmVenda.qrprodutos_vr.sql.add('where pro.codigo = est.codproduto and pro.codigo = :cod');
      frmVenda.qrprodutos_vr.sql.add('and pro.SITUACAO = 0');
      frmVenda.qrprodutos_vr.sql.add('order by pro.produto');
      frmVenda.qrprodutos_vr.ParamByName('cod').AsString := qrprodutosSV.fieldbyname('codigo').AsString;//zerar(ed_barra.Text,6);
      frmVenda.qrprodutos_vr.Open;

    if frmVenda.qrprodutos_vr.recordcount > 0 then
      Begin
       if frmVenda.qrconfig.fieldbyname('NAOVENDERSEMESTOQUE').AsString = '1' then
      Begin
       if frmVenda.qrprodutos_vr.fieldbyname('ESTOQUE_ATUAL').asfloat <= 0 then
       begin

          frmVenda.ED_barra.TEXT := frmVenda.qrprodutos_vr.FieldByName('produto').AsString;
...
begin
  ...
  if not bLanca_pre_venda then
    if (iECF_Modelo <> BEMATECH) and (iECF_Modelo <> EPSON) then
      sNumero_cupom := Zerar(inttostr(strtoint(snumero_cupom) - 1), 10);


  for i := 0 to 2 do
  begin
    sNumero_cupom := zerar(inttostr(strtoint(sNumero_cupom) - i), 10);
...
{  if img_produto.Picture.Bitmap <> nil then begin
  ...
  sMsg := Imp_Vende_item(sPortaNaoFiscal, Zerar(inttostr(iProd_codigo), 12),
    sProd_Nome,
...
begin
  ...
  try
    ...
    begin
      ...
      sNumero_Cupom := frmPrincipal.adic_codifica('000048');//Zerar(FloatToStr(Max('')), 9);
      sNumero_contador_cupom := Zerar(sNumero_Cupom, 10);

      if Length(sNumero_Cupom) = 9 then
        sNumero_Cupom := '9' + sNumero_Cupom;

      if Length(sNumero_contador_cupom) = 9 then
...
begin
  ...
  begin
    Imprime_display_teclado('ACTHAUROS', 'ACTHAUROS');
...
begin
  ...
  if iTeclado_modelo = 1 then begin
    // abrir a comunicacao
    OpenTec44;
    Imprime_display_teclado('ACTHAUROS V-2023', 'Terminal iniciado com sucesso!');
  end;
...
begin
  ...
  begin
   TimerVerificar_operador.Enabled := False;
    ...
    if lbcod_operador.Caption = '000000' then
    Begin
       rocarOperador1.Click;
    End;
   ...
   frmPrincipal.dataini.Text := FormatDateTime('dd/mm/yyyy',Now);

   frmmodulo.query.sql.clear;
   frmmodulo.query.sql.add('select * from config');
   frmmodulo.query.open;

   if frmmodulo.query.fieldbyname('CARGA_DATA').AsDateTime < strtodatetime(frmPrincipal.dataini.Text) then
   begin
    Begin
      frmPrincipal.qrverificar_dataSV.CLOSE;
      frmPrincipal.qrverificar_dataSV.SQL.CLEAR;
      frmPrincipal.qrverificar_dataSV.SQl.ADD ('UPDATE C000045 SET');
      frmPrincipal.qrverificar_dataSV.SQl.ADD ('SITUACAO_ATUAL =:UP, DATA  = ''' + FormatDateTime('dd.mm.yyyy',Now)+ '''');
      frmPrincipal.qrverificar_dataSV.ParamByName('UP').AsString := 'CAIXA ABERTO';
      frmPrincipal.qrverificar_dataSV.ExecSQL;
     End;

    Begin
     frmPrincipal.qrverificar_data.CLOSE;
     frmPrincipal.qrverificar_data.SQL.CLEAR;
     frmPrincipal.qrverificar_data.SQl.ADD ('UPDATE CONFIG SET');
     frmPrincipal.qrverificar_data.SQl.ADD ('CARGA_DATA = ''' + FormatDateTime('dd/mm/yyyy',Now)+ '''');
     frmPrincipal.qrverificar_data.ExecSQL;
    End;
    ShowMessage('Data e hora está diferente do servidor,'
                       +#13+ 'Seu sistema será reiniciado automaticamente!');

     WinExec('C:\ACTHAUROS\PDV\renicia_PC.bat', SW_SHOW);
     CLOSE;
     FRMVENDA.CLOSE;
     FRMPRINCIPAL.CLOSE;
...
begin
  ...
  if Pn_gerar_nfce.Visible = True then
  Begin

    frmMsg_Operador.lb_msg.caption := 'Aguarde!Finalizando NFCe!...';
...
begin
    ...
    dxBarCode_QrCode_pix.Visible := False;
     ...
     qrfilial.open;
      ...
      begin
       frmPrincipal.TipoImpressora := semImpressora; //ativar nfce
       CorEditTotaL;
      end;
      if qrfilial.fieldbyname('CONTADOR_FAX').asstring = '2' then
      begin
       frmPrincipal.TipoImpressora := NaoFiscal;   //desativa nfce
       CorEditTotaL;
      end;
...
procedure TfrmVenda.dxBarCode_QrCode_pixClick(Sender: TObject);
begin
  ...
  begin
    qrpix.close;
    qrpix.sql.clear;
    qrpix.sql.add('update CUPOM_ITEM_TEMP set CHAVE_PIX_PG =:PIX');
    qrpix.sql.add('where item = 1 and cod_cupom = ''' +
    sNumero_Cupom + '''');
    qrpix.ParamByName('PIX').AsString  := edtpayload.Text;
    qrpix.ExecSQL;
  end;
  ...
  qrpix_ver.close;
  qrpix_ver.sql.clear;
  qrpix_ver.sql.add('select TEMP.*');
  qrpix_ver.sql.add('FROM CUPOM_ITEM_TEMP TEMP');
  qrpix_ver.sql.add('where TEMP.item = 1 and ');
  qrpix_ver.sql.add('TEMP.cod_cupom = ''' +
  sNumero_Cupom + '''');
  qrpix_ver.ExecSQL;
  //query.open;
  End;
 ...
 // TfrxPictureView(Frx_PreContax.FindObject('Barcode2D1')).T ('C:\ACTHAUROS\PDV\Img\QRCOD_empresa.jpg');
//begin
  // inherited;
  Frx_PreConta_pix.PrepareReport;
  Frx_PreConta_pix.Print;
end;
      ...
      if (AnsiUpperCase(cb_forma1.text) = AnsiUpperCase(lForma_pgto_Cartao_Debito)) or
        ...
        Begin
         ...
         qrconfig.open;
          ...
          if qrconfig.fieldbyname('ATIVAR_NFCE_FORMA_CARTAO').AsString = '1' then
          Begin
           frmPrincipal.TipoImpressora := semImpressora; //ativar nfce
           CorEditTotaL;
          end;
          if qrconfig.fieldbyname('ATIVAR_NFCE_FORMA_CARTAO').AsString = '0' then
          begin
           frmPrincipal.TipoImpressora := NaoFiscal;   //desativa nfce
           CorEditTotaL;
          end;
            ...
            repeat
              ...
              begin
                ...
                sMsg := cECF_Usa_Gerencial(iECF_Modelo,
                  ...
                  zerar(sNumero_Cupom, 10), 48, ' ', taEsquerda));
                sMsg := cECF_Usa_Gerencial(iECF_Modelo,
                  Texto_Justifica('DOCUMENTO.: ' +
                  zerar(sNumero_Cupom, 10) + '/' + inttostr(i), 48,
                  ' ', taEsquerda));
                sMsg := cECF_Usa_Gerencial(iECF_Modelo,
                  '------------------------------------------------');
                sMsg := cECF_Usa_Gerencial(iECF_Modelo,
              ...
              begin // Nao Fiscal
                ...
                sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,
                  Texto_Justifica('CUPOM.....: ' +
                  zerar(sNumero_Cupom, 10), 41, ' ', taEsquerda));
                sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,
                  Texto_Justifica('DOCUMENTO.: ' +
                  zerar(sNumero_Cupom, 10) + '/' + inttostr(i), 41,
                  ' ', taEsquerda));
                sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,
                  '-----------------------------------------');
                sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,
 ...
 End;
        ...
        begin
          ...
          begin
              ...
              qrconfig.ExecSQL;
               ...
               qrcomprovantedebito.close;
               qrcomprovantedebito.sql.clear;
               qrcomprovantedebito.sql.add('select cred.*, clie.nome, clie.endereco, clie.cpf');
               qrcomprovantedebito.sql.add('from cupom_crediario cred, cliente clie');
               qrcomprovantedebito.sql.add('where cred.cod_cliente = clie.codigo and cred.descricao = ''APRAZO'' ');
               qrcomprovantedebito.sql.add('and cred.COD_CUPOM = ''' + sCod_Cupom + '''');
               qrcomprovantedebito.ExecSQL;

               if qrconfig.fieldbyname('A4_CUPOM').asstring = '2' then
               Begin
               frxcomprovantedebitoA4.PrepareReport;
               frxcomprovantedebitoA4.Print;
               End;

               if qrconfig.fieldbyname('A4_CUPOM').asstring = '1' then
               Begin
               frxcomprovantedebito.PrepareReport;
               frxcomprovantedebito.Print;
               End;
          end;
        end;
  ...
  begin
   ...
   begin
          ...
          if qrconfig.fieldbyname('A4_CUPOM').asstring = '1' then
        Begin
         TfrxPictureView(frmVenda.Frx_ImpressaoVenda.FindObject('Picture1')).Picture.LoadFromFile('C:\ACTHAUROS\PDV\Img\QRCOD_empresa.jpg');
        ...
        if qrconfig.fieldbyname('A4_CUPOM').asstring = '2' then
        Begin
         TfrxPictureView(frmVenda.Frx_ImpressaoVendaA4.FindObject('Picture1')).Picture.LoadFromFile('C:\ACTHAUROS\PDV\Img\QRCOD_empresa.jpg');
        ...
        Begin
          ...
          frmVenda.Frx_ImpressaoCompPix.Print;
        End;
        if cb_forma2.ItemIndex = 1 then
        Begin
          frmVenda.UniQuery1.close;
...
begin
  ...
  begin
    try
      ...
      frmMsg_Operador.lb_msg.caption := 'AGUARDE...  GERANDO NFC-E';
        frmMsg_Operador.Show;
        frmMsg_Operador.Refresh;
        Application.ProcessMessages;//Imprime_display('AGUARDE...  GERANDO NFC-E', CLWHITE, tiLivre);
      grid.Repaint;
      GerarNFCe(vAux);
       frmMsg_Operador.Hide;

        frmMsg_Operador.lb_msg.caption := 'AGUARDE...  ASSINANDO NFC-E';
        frmMsg_Operador.Show;
        frmMsg_Operador.Refresh;
        Application.ProcessMessages;//Imprime_display('AGUARDE...  ASSINANDO NFC-E', CLWHITE, tiLivre);
      grid.Repaint;
      ACBRNFCe.NotasFiscais.Assinar;
      frmMsg_Operador.Hide;

        frmMsg_Operador.lb_msg.caption := 'AGUARDE...  VALIDANDO NFC-E';
        frmMsg_Operador.Show;
        frmMsg_Operador.Refresh;
        Application.ProcessMessages;//Imprime_display('AGUARDE...  VALIDANDO NFC-E', CLWHITE, tiLivre);
      grid.Repaint;
      ACBRNFCe.NotasFiscais.Validar;
      frmMsg_Operador.Hide;

      if ACBRNFCe.Configuracoes.Geral.FormaEmissao <> frmtOffLine then begin
        frmMsg_Operador.lb_msg.caption := 'AGUARDE...  ENVIANDO NFC-E';
        frmMsg_Operador.Show;
        frmMsg_Operador.Refresh;
        Application.ProcessMessages;//Imprime_display('AGUARDE...  ENVIANDO NFC-E', CLWHITE, tiLivre);
        grid.Repaint;
        try
        ...
        if ACBRNFCe.NotasFiscais.Items[0].Confirmada then begin
          ...
          frmMsg_Operador.lb_msg.caption := ACBRNFCe.WebServices.Enviar.xMotivo;
          frmMsg_Operador.Show;
          frmMsg_Operador.Refresh;
          Application.ProcessMessages;//Imprime_display(ACBRNFCe.WebServices.Enviar.xMotivo, CLWHITE, tiLivre);
        end;
      end else begin
        ChaveNFCE := ACBRNFCe.NotasFiscais.Items[0].NFe.infNFe.Id;
        NumeroNFCe := strtoint(vAux);
   ...
   // bTef_finaliza := false;
    ...
    begin
      try
        ...
        frmMsg_Operador.lb_msg.caption := 'AGUARDE...  PREPARANDO NFC-E...';
        frmMsg_Operador.Show;
        frmMsg_Operador.Refresh;
        Application.ProcessMessages;
        ...
        frmMsg_Operador.lb_msg.caption := 'AGUARDE...  GRAVANDO NFC-E NO BANCO...';
        frmMsg_Operador.Show;
        frmMsg_Operador.Refresh;
        Application.ProcessMessages;
        {Imprime_display('AGUARDE...  GRAVANDO NFC-E NO BANCO',
                 ...
                 begin
                  ...
                  Begin
                    ...
                    Begin
                      ...
                      qrnfce_servidor.sql.add('update c000048 set gerado_nfce =:S where NUMERO_CUPOM_FISCAL =:cod'); //''' + sNumero_cupom + '''');
                      qrnfce_servidor.ParamByName('S').AsString := 'S';
                      qrnfce_servidor.ParamByName('cod').AsString := lbcodvenda.Caption;//frmmodulo.qrvendanfce.fieldbyname('NUMERO_CUPOM_FISCAL').asstring;
...
procedure TfrmVenda.btgerar_pixClick(Sender: TObject);
  var
 QrCode :String;
 scod_cupom: string;
begin
  ...
  Begin
   ...
   RzLabel5.Caption := 'Chave Pix.: '+edtChave.Text;
   edtCidade.Text := frmvenda.qremitente.FieldByName('CIDADE').asstring;

   edtCodTrans.Text := '';
   edtpayload.Text := '';
  End;

  if frmvenda.cb_forma1.ItemIndex = 1 then
  Begin
   edtValor.Text := formatfloat('########0.00', frmvenda.ed_forma1.Value);
  End;

  if frmvenda.cb_forma2.ItemIndex = 1 then
  Begin
   edtValor.Text := formatfloat('########0.00', frmvenda.ed_forma2.Value);
  End;


  if frmvenda.cb_forma3.ItemIndex = 1 then
  Begin
   edtValor.Text := formatfloat('########0.00', frmvenda.ed_forma3.Value);
  End;

   edtCodTrans.Text := 'NV'+sNumero_cupom;
...
function TfrmVenda.TrocaVirgPPto(Valor: string): String;
   var i:integer;
begin
      if Valor <>'' then begin
...
begin
 ...
 // CentralizarPanel(pnFundo);
  ...
  sImgFundo := ExtractFilePath(Application.ExeName) + 'img\fundo_venda.jpg';
    ...
    imagem_empresa.Picture.LoadFromFile('C:\ACTHAUROS\PDV\img\imagen_logo.png');
 ...
 //   imgFundo.Picture.LoadFromFile(sImgFundo);
  ...
  begin
    ...
    begin
      ...
      query.close;
      query.sql.clear;
      query.sql.add('select item.*');
      query.sql.add('from cupom_item_temp item');
    ...
    begin
      ...
      begin
        sNumero_Cupom := Zerar(FloatToStr(Max('')), 10);
        sNumero_contador_cupom := Zerar(sNumero_Cupom, 10);
      end;
...
begin
      ...
      frmStatusNFCe := TfrmStatusNFCe.create(self);
...
begin
 ...
 //  if Menu_os.Visible = false then exit;
   if lbcod_operador.Caption = '000000' then
    ...
    if lbcod_operador.Caption = '' then
    Begin
       rocarOperador1.Click;
    End;
