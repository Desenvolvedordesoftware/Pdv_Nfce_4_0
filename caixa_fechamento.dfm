object frmCaixa_Fechamento: TfrmCaixa_Fechamento
  Left = 225
  Top = 56
  BorderStyle = bsNone
  Caption = 'Caixa - Fechamento'
  ClientHeight = 248
  ClientWidth = 291
  Color = clWhite
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  PopupMenu = pop_fechamento
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 32
    Top = 33
    Width = 321
    Height = 187
    BevelOuter = bvNone
    Color = clWhite
    ParentBackground = False
    TabOrder = 0
    object lbEdicao: TLabel
      Left = 50
      Top = 350
      Width = 31
      Height = 13
      Caption = 'Edi'#231#227'o'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Visible = False
    end
    object Label4: TLabel
      Left = 11
      Top = 42
      Width = 105
      Height = 13
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Data do Movimento:'
    end
    object Label6: TLabel
      Left = 247
      Top = 22
      Width = 65
      Height = 13
      Alignment = taCenter
      AutoSize = False
      Caption = 'Horas Inicio:'
      Visible = False
    end
    object Label7: TLabel
      Left = 247
      Top = 59
      Width = 65
      Height = 13
      Alignment = taCenter
      AutoSize = False
      Caption = 'Horas Fim:'
      Visible = False
    end
    object bt_ok: TButton
      Left = 129
      Top = 379
      Width = 90
      Height = 27
      Caption = '&Fechar Caixa'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = bt_okClick
    end
    object AdvGlowButton1: TAdvGlowButton
      Left = 12
      Top = 91
      Width = 93
      Height = 77
      Caption = 'Fechar Caixa'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        FF1B00A2177EFF42865FD10000000049454E44AE426082}
      Transparent = True
      TabOrder = 1
      OnClick = bt_okClick
      Appearance.BorderColor = 12631218
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 14671574
      Appearance.ColorTo = 15000283
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 14144974
      Appearance.ColorMirrorTo = 15197664
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
      Layout = blGlyphTop
    end
    object AdvGlowButton2: TAdvGlowButton
      Left = 119
      Top = 80
      Width = 93
      Height = 88
      Caption = 'Relatorio Simples'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        53E93D8C0000000049454E44AE426082}
      Transparent = True
      TabOrder = 2
      OnClick = AdvGlowButton2Click
      Appearance.BorderColor = 12631218
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 14671574
      Appearance.ColorTo = 15000283
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 14144974
      Appearance.ColorMirrorTo = 15197664
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
      Layout = blGlyphTop
    end
    object ed_data: TJvDateEdit
      Left = 122
      Top = 40
      Width = 87
      Height = 19
      DateFormat = 'DD/MM/YYYY'
      Color = clWhite
      Flat = True
      ParentFlat = False
      NumGlyphs = 2
      ShowNullDate = False
      TabOrder = 3
      OnChange = ed_dataChange
    end
    object AdvGlowButton3: TAdvGlowButton
      Left = 356
      Top = 67
      Width = 93
      Height = 88
      Caption = 'Relatorio Vendas canceladas'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        53E93D8C0000000049454E44AE426082}
      Transparent = True
      TabOrder = 4
      Visible = False
      OnClick = AdvGlowButton3Click
      Appearance.BorderColor = 12631218
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 14671574
      Appearance.ColorTo = 15000283
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 14144974
      Appearance.ColorMirrorTo = 15197664
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
      Layout = blGlyphTop
    end
    object AdvGlowButton4: TAdvGlowButton
      Left = 284
      Top = 99
      Width = 93
      Height = 88
      Caption = 'Rel. Simples por Hs'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        53E93D8C0000000049454E44AE426082}
      Transparent = True
      TabOrder = 5
      Visible = False
      OnClick = AdvGlowButton4Click
      Appearance.BorderColor = 12631218
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 14671574
      Appearance.ColorTo = 15000283
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 14144974
      Appearance.ColorMirrorTo = 15197664
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
      Layout = blGlyphTop
    end
    object TimeEdit_IN: TcxTimeEdit
      Left = 247
      Top = 35
      TabOrder = 6
      Visible = False
      Width = 70
    end
    object TimeEdit_FI: TcxTimeEdit
      Left = 247
      Top = 72
      TabOrder = 7
      Visible = False
      Width = 70
    end
  end
  object Panel2: TPanel
    Left = 16
    Top = 47
    Width = 233
    Height = 23
    BevelOuter = bvNone
    Color = clWhite
    Enabled = False
    ParentBackground = False
    TabOrder = 1
    object Label1: TLabel
      Left = 365
      Top = 29
      Width = 105
      Height = 13
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Data do Movimento:'
    end
    object Label2: TLabel
      Left = 23
      Top = 5
      Width = 59
      Height = 13
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Operador:'
      Visible = False
    end
    object Label3: TLabel
      Left = 0
      Top = 31
      Width = 105
      Height = 13
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'N'#250'mero do ECF:'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Visible = False
    end
    object ed_data1: TJvDateEdit
      Left = 50
      Top = 70
      Width = 81
      Height = 19
      Color = 15916445
      Flat = True
      ParentFlat = False
      NumGlyphs = 2
      ShowNullDate = False
      TabOrder = 0
    end
    object ed_ecf: TRzEdit
      Left = 112
      Top = 28
      Width = 321
      Height = 19
      Text = ''
      Color = 15916445
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 1
      Visible = False
    end
    object bt_cupom_encerrante: TButton
      Left = 924
      Top = 22
      Width = 75
      Height = 25
      Caption = 'Cp Encerrante'
      TabOrder = 2
      Visible = False
      OnClick = bt_cupom_encerranteClick
    end
    object ed_operador: TRzEdit
      Left = 88
      Top = 2
      Width = 137
      Height = 19
      Text = ''
      Color = 15916445
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 3
      Visible = False
    end
  end
  object PageControl1: TPageControl
    Left = 1
    Top = 384
    Width = 632
    Height = 141
    ActivePage = TabSheet1
    TabOrder = 2
    object TabSheet1: TTabSheet
      Caption = 'Resumo'
    end
    object TabSheet2: TTabSheet
      Caption = 'Formas Pagto.'
      ImageIndex = 1
    end
    object TabSheet3: TTabSheet
      Caption = 'Al'#237'quotas'
      ImageIndex = 2
    end
    object TabSheet4: TTabSheet
      Caption = 'Outros'
      ImageIndex = 3
    end
    object TabSheet5: TTabSheet
      Caption = 'Pr'#233'-vendas'
      ImageIndex = 4
    end
    object TabSheet6: TTabSheet
      Caption = 'DAVs'
      ImageIndex = 5
    end
    object TabSheet7: TTabSheet
      Caption = 'Abast.Pendentes'
      ImageIndex = 6
      object Panel3: TPanel
        Left = 0
        Top = 72
        Width = 624
        Height = 41
        Align = alBottom
        BevelOuter = bvNone
        TabOrder = 0
        object Button1: TButton
          Left = 176
          Top = 8
          Width = 161
          Height = 25
          Caption = 'Finalizar Todos'
          TabOrder = 0
          OnClick = Button1Click
        end
      end
    end
    object TabSheet8: TTabSheet
      Caption = 'Mesas Abert.'
      ImageIndex = 7
    end
    object TabFechamento: TTabSheet
      Caption = 'Fechamento'
      ImageIndex = 8
    end
  end
  object AdvSmoothExpanderPanel1: TAdvSmoothExpanderPanel
    AlignWithMargins = True
    Left = 3
    Top = 3
    Width = 285
    Height = 38
    Cursor = crDefault
    Caption.HTMLFont.Charset = DEFAULT_CHARSET
    Caption.HTMLFont.Color = clWindowText
    Caption.HTMLFont.Height = -11
    Caption.HTMLFont.Name = 'Tahoma'
    Caption.HTMLFont.Style = []
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clWindowText
    Caption.Font.Height = -16
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    Caption.Line = False
    Fill.Color = 6960384
    Fill.ColorTo = clNone
    Fill.ColorMirror = clNone
    Fill.ColorMirrorTo = clNone
    Fill.GradientType = gtSolid
    Fill.GradientMirrorType = gtSolid
    Fill.BorderColor = clNone
    Fill.BorderWidth = 0
    Fill.Rounding = 0
    Fill.ShadowColor = clNone
    Fill.ShadowOffset = 0
    Fill.Glow = gmNone
    Version = '1.1.1.0'
    Align = alTop
    TabOrder = 3
    ShowExpander = False
    ExpanderColor = 16445929
    ExpanderDownColor = 15587527
    ExpanderHoverColor = 11196927
    TMSStyle = 0
    OldHeight = 38.000000000000000000
    object Label53: TLabel
      AlignWithMargins = True
      Left = 0
      Top = 8
      Width = 236
      Height = 23
      Alignment = taCenter
      AutoSize = False
      Caption = 'Movimenta'#231#227'o do caixa'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -19
      Font.Name = 'Verdana'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object AdvMetroButton1: TAdvMetroButton
      Left = 229
      Top = 0
      Width = 56
      Height = 38
      Align = alRight
      Appearance.PictureColor = clWhite
      Caption = ''
      Picture.Data = {
        
        65B5119A54FB20C0004E7B10B44195D3E70000000049454E44AE426082}
      TabOrder = 0
      Version = '1.1.2.0'
      OnClick = AdvMetroButton1Click
    end
  end
  object Panel4: TPanel
    Left = -11
    Top = 384
    Width = 668
    Height = 45
    BevelOuter = bvNone
    Color = 12615680
    ParentBackground = False
    TabOrder = 4
    Visible = False
    object bt_fechamento01: TAdvGlowButton
      Left = 8
      Top = 6
      Width = 88
      Height = 33
      Caption = 'Fechamento'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        5D0000000049454E44AE426082}
      Transparent = True
      TabOrder = 0
      OnClick = bt_fechamento01Click
      Appearance.BorderColor = 12631218
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 14671574
      Appearance.ColorTo = 15000283
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 14144974
      Appearance.ColorMirrorTo = 15197664
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
    object bt_fechamento02: TAdvGlowButton
      Left = 98
      Top = 6
      Width = 100
      Height = 33
      Caption = 'Mesas Abertas'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        5D0000000049454E44AE426082}
      Transparent = True
      TabOrder = 1
      OnClick = bt_fechamento02Click
      Appearance.BorderColor = 12631218
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 14671574
      Appearance.ColorTo = 15000283
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 14144974
      Appearance.ColorMirrorTo = 15197664
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
    object bt_fechamento03: TAdvGlowButton
      Left = 200
      Top = 6
      Width = 64
      Height = 33
      Caption = 'DAVs'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        5D0000000049454E44AE426082}
      Transparent = True
      TabOrder = 2
      OnClick = bt_fechamento03Click
      Appearance.BorderColor = 12631218
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 14671574
      Appearance.ColorTo = 15000283
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 14144974
      Appearance.ColorMirrorTo = 15197664
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
    object bt_fechamento04: TAdvGlowButton
      Left = 266
      Top = 6
      Width = 85
      Height = 33
      Caption = 'Pr'#233' Vendas'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        5D0000000049454E44AE426082}
      Transparent = True
      TabOrder = 3
      OnClick = bt_fechamento04Click
      Appearance.BorderColor = 12631218
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 14671574
      Appearance.ColorTo = 15000283
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 14144974
      Appearance.ColorMirrorTo = 15197664
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
    object bt_fechamento05: TAdvGlowButton
      Left = 353
      Top = 6
      Width = 65
      Height = 33
      Caption = 'Outros'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        5D0000000049454E44AE426082}
      Transparent = True
      TabOrder = 4
      OnClick = bt_fechamento05Click
      Appearance.BorderColor = 12631218
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 14671574
      Appearance.ColorTo = 15000283
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 14144974
      Appearance.ColorMirrorTo = 15197664
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
    object bt_fechamento06: TAdvGlowButton
      Left = 420
      Top = 6
      Width = 76
      Height = 33
      Caption = 'Aliquotas'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        5D0000000049454E44AE426082}
      Transparent = True
      TabOrder = 5
      OnClick = bt_fechamento06Click
      Appearance.BorderColor = 12631218
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 14671574
      Appearance.ColorTo = 15000283
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 14144974
      Appearance.ColorMirrorTo = 15197664
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
    object bt_fechamento07: TAdvGlowButton
      Left = 498
      Top = 6
      Width = 106
      Height = 33
      Caption = 'F. Pagamentos'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        5D0000000049454E44AE426082}
      Transparent = True
      TabOrder = 6
      OnClick = bt_fechamento07Click
      Appearance.BorderColor = 12631218
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 14671574
      Appearance.ColorTo = 15000283
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 14144974
      Appearance.ColorMirrorTo = 15197664
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
    object bt_fechamento08: TAdvGlowButton
      Left = 606
      Top = 6
      Width = 69
      Height = 33
      Caption = 'Resumo'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      Picture.Data = {
        
        5D0000000049454E44AE426082}
      Transparent = True
      TabOrder = 7
      OnClick = bt_fechamento08Click
      Appearance.BorderColor = 12631218
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 14671574
      Appearance.ColorTo = 15000283
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 14144974
      Appearance.ColorMirrorTo = 15197664
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
  end
  object Panel5: TPanel
    Left = -24
    Top = 525
    Width = 745
    Height = 38
    BevelOuter = bvNone
    Color = 12615680
    ParentBackground = False
    TabOrder = 5
  end
  object Combo_Usuario: TComboBox
    Left = 449
    Top = 100
    Width = 180
    Height = 21
    Color = 15916445
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 6
  end
  object AdvSmoothExpanderPanel2: TAdvSmoothExpanderPanel
    AlignWithMargins = True
    Left = 3
    Top = 207
    Width = 285
    Height = 38
    Cursor = crDefault
    Caption.HTMLFont.Charset = DEFAULT_CHARSET
    Caption.HTMLFont.Color = clWindowText
    Caption.HTMLFont.Height = -11
    Caption.HTMLFont.Name = 'Tahoma'
    Caption.HTMLFont.Style = []
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clWindowText
    Caption.Font.Height = -16
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    Caption.Line = False
    Fill.Color = 6960384
    Fill.ColorTo = clNone
    Fill.ColorMirror = clNone
    Fill.ColorMirrorTo = clNone
    Fill.GradientType = gtSolid
    Fill.GradientMirrorType = gtSolid
    Fill.BorderColor = clNone
    Fill.BorderWidth = 0
    Fill.Rounding = 0
    Fill.ShadowColor = clNone
    Fill.ShadowOffset = 0
    Fill.Glow = gmNone
    Version = '1.1.1.0'
    Align = alBottom
    TabOrder = 7
    ShowExpander = False
    ExpanderColor = 16445929
    ExpanderDownColor = 15587527
    ExpanderHoverColor = 11196927
    TMSStyle = 0
    OldHeight = 38.000000000000000000
  end
  object AdvSmoothExpanderPanel3: TAdvSmoothExpanderPanel
    AlignWithMargins = True
    Left = 3
    Top = 47
    Width = 38
    Height = 154
    Cursor = crDefault
    Caption.HTMLFont.Charset = DEFAULT_CHARSET
    Caption.HTMLFont.Color = clWindowText
    Caption.HTMLFont.Height = -11
    Caption.HTMLFont.Name = 'Tahoma'
    Caption.HTMLFont.Style = []
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clWindowText
    Caption.Font.Height = -16
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    Caption.Line = False
    Fill.Color = 6960384
    Fill.ColorTo = clNone
    Fill.ColorMirror = clNone
    Fill.ColorMirrorTo = clNone
    Fill.GradientType = gtSolid
    Fill.GradientMirrorType = gtSolid
    Fill.BorderColor = clNone
    Fill.BorderWidth = 0
    Fill.Rounding = 0
    Fill.ShadowColor = clNone
    Fill.ShadowOffset = 0
    Fill.Glow = gmNone
    Version = '1.1.1.0'
    Align = alLeft
    TabOrder = 8
    ShowExpander = False
    ExpanderColor = 16445929
    ExpanderDownColor = 15587527
    ExpanderHoverColor = 11196927
    TMSStyle = 0
    OldHeight = 154.000000000000000000
  end
  object AdvSmoothExpanderPanel4: TAdvSmoothExpanderPanel
    AlignWithMargins = True
    Left = 250
    Top = 47
    Width = 38
    Height = 154
    Cursor = crDefault
    Caption.HTMLFont.Charset = DEFAULT_CHARSET
    Caption.HTMLFont.Color = clWindowText
    Caption.HTMLFont.Height = -11
    Caption.HTMLFont.Name = 'Tahoma'
    Caption.HTMLFont.Style = []
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clWindowText
    Caption.Font.Height = -16
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    Caption.Line = False
    Fill.Color = 6960384
    Fill.ColorTo = clNone
    Fill.ColorMirror = clNone
    Fill.ColorMirrorTo = clNone
    Fill.GradientType = gtSolid
    Fill.GradientMirrorType = gtSolid
    Fill.BorderColor = clNone
    Fill.BorderWidth = 0
    Fill.Rounding = 0
    Fill.ShadowColor = clNone
    Fill.ShadowOffset = 0
    Fill.Glow = gmNone
    Version = '1.1.1.0'
    Align = alRight
    TabOrder = 9
    ShowExpander = False
    ExpanderColor = 16445929
    ExpanderDownColor = 15587527
    ExpanderHoverColor = 11196927
    TMSStyle = 0
    OldHeight = 154.000000000000000000
  end
  object pop_fechamento: TAdvPopupMenu
    MenuStyler = frmModulo.estilo_menu
    Version = '2.6.2.1'
    Left = 373
    Top = 374
    object Cancelar1: TMenuItem
      Caption = 'Cancelar'
      ShortCut = 27
      OnClick = Cancelar1Click
    end
  end
  object qrAbastecimento: TUniQuery
    Connection = frmModulo.conexao
    Left = 437
    Top = 462
  end
  object qrMesa: TUniQuery
    Connection = frmModulo.Conexao_Servidor
    SQL.Strings = (
      'select '
      '  c000074.*,'
      '  c000007.Nome Cliente,'
      '  c000008.Nome Vendedor'
      'from '
      '  c000074, c000007, c000008'
      'where'
      '  c000074.codcliente = c000007.codigo and'
      '  c000074.codvendedor = c000008.codigo')
    Left = 509
    Top = 414
  end
  object qrEncerrante: TUniQuery
    Connection = frmModulo.conexao
    Left = 557
    Top = 422
  end
  object qrDAV: TUniQuery
    Connection = frmModulo.conexao
    Left = 437
    Top = 422
  end
  object query: TUniQuery
    Connection = frmModulo.conexao
    Left = 309
    Top = 438
  end
  object qrPre_Venda: TUniQuery
    Connection = frmModulo.Conexao_Servidor
    SQL.Strings = (
      'select '
      '  c000074.*,'
      '  c000007.Nome Cliente,'
      '  c000008.Nome Vendedor'
      'from '
      '  c000074, c000007, c000008'
      'where'
      '  c000074.codcliente = c000007.codigo and'
      '  c000074.codvendedor = c000008.codigo')
    Left = 461
    Top = 302
  end
  object qrArquivo: TUniQuery
    Connection = frmModulo.conexao
    Left = 565
    Top = 358
  end
  object qrFechamento: TUniQuery
    Connection = frmModulo.conexao
    Left = 637
    Top = 310
  end
  object frxDesigner1: TfrxDesigner
    DefaultScriptLanguage = 'PascalScript'
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -13
    DefaultFont.Name = 'Arial'
    DefaultFont.Style = []
    DefaultLeftMargin = 10.000000000000000000
    DefaultRightMargin = 10.000000000000000000
    DefaultTopMargin = 10.000000000000000000
    DefaultBottomMargin = 10.000000000000000000
    DefaultPaperSize = 9
    DefaultOrientation = poPortrait
    GradientEnd = 11982554
    GradientStart = clWindow
    TemplatesExt = 'fr3'
    Restrictions = []
    RTLLanguage = False
    MemoParentFont = False
    OnGetTemplateList = frxDesigner1GetTemplateList
    Left = 272
    Top = 520
  end
  object fxFechamento: TfrxReport
    Version = '5.4.6'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintMode = pmSplit
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 42970.860474629600000000
    ReportOptions.LastChange = 43626.273487407400000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'procedure MasterData2OnBeforePrint(Sender: TfrxComponent);'
      'begin'
      '  if m3.text = '#39'S'#39' then begin'
      '    m1.Font.Style := fsBold;'
      '    m2.Font.Style := fsBold;'
      '  end else begin'
      '    m1.Font.Style := 0;'
      '    m2.Font.Style := 0;'
      '  end;'
      'end;'
      ''
      'begin'
      ''
      'end.')
    Left = 384
    Top = 168
    Datasets = <
      item
        DataSet = frxDados
        DataSetName = 'frDados'
      end
      item
        DataSet = frxEmitente
        DataSetName = 'frEmitente'
      end
      item
        DataSet = frxDBaprazo
        DataSetName = 'frxDBaprazo'
      end
      item
        DataSet = frxdbdtst1
        DataSetName = 'frxDBDataset1'
      end
      item
        DataSet = frxDBcartaoc
        DataSetName = 'frxDBcartaoc'
      end
      item
        DataSet = frxDBcartaod
        DataSetName = 'frxDBcartaod'
      end
      item
        DataSet = frxDBdinheiro
        DataSetName = 'frxDBdinheiro'
      end
      item
        DataSet = frxDBDFechamento
        DataSetName = 'frxDBDFechamento'
      end
      item
        DataSet = frxDBDsangria
        DataSetName = 'frxDBDsangria'
      end
      item
        DataSet = frxDBDsuprimento
        DataSetName = 'frxDBDsuprimento'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 60.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 256
      LeftMargin = 5.000000000000000000
      RightMargin = 5.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = []
        Height = 162.519790000000000000
        ParentFont = False
        Top = 18.897650000000000000
        Width = 188.976500000000000000
        object Memo2: TfrxMemoView
          Align = baCenter
          Left = 68.031540000000000000
          Width = 52.913420000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."FANTASIA"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo1: TfrxMemoView
          Align = baCenter
          Left = 71.811070000000000000
          Top = 18.897650000000000000
          Width = 45.354360000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[<frEmitente."ENDERECO">], [<frEmitente."NUMERO">]')
          ParentFont = False
          WordWrap = False
        end
        object Memo3: TfrxMemoView
          Align = baCenter
          Left = 47.244125000000000000
          Top = 34.015770000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."CIDADE"] / [frEmitente."UF"] - [frEmitente."CEP"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo4: TfrxMemoView
          Align = baCenter
          Left = 47.244125000000000000
          Top = 49.133890000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."TELEFONE"]')
          ParentFont = False
          WordWrap = False
          Formats = <
            item
            end
            item
            end
            item
            end>
        end
        object Memo5: TfrxMemoView
          Align = baCenter
          Left = 66.141775000000000000
          Top = 64.252010000000000000
          Width = 56.692950000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."CNPJ"] - [frEmitente."IE"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo6: TfrxMemoView
          Align = baLeft
          Top = 117.165430000000000000
          Width = 117.165430000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Data: [Date]')
          ParentFont = False
          WordWrap = False
        end
        object mCaixa: TfrxMemoView
          Top = 139.842610000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Caixa: 001')
          ParentFont = False
          WordWrap = False
          Formats = <
            item
            end
            item
            end>
        end
        object mOperador: TfrxMemoView
          Left = 81.259895000000000000
          Top = 139.842610000000000000
          Width = 102.047310000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Operador: [frxDBDataset1."CODFUNCIONARIO"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo9: TfrxMemoView
          Align = baCenter
          Left = -35.905535000000000000
          Top = 86.929190000000000000
          Width = 260.787570000000000000
          Height = 22.677180000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '*** FECHAMENTO CAIXA ***')
          ParentFont = False
          WordWrap = False
        end
        object Memo10: TfrxMemoView
          Align = baRight
          Left = 71.811070000000000000
          Top = 113.385900000000000000
          Width = 117.165430000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Hor'#225'rio: [Time]')
          ParentFont = False
          WordWrap = False
        end
        object Line2: TfrxLineView
          Align = baCenter
          Left = -56.692950000000000000
          Top = 162.519790000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
        object Line1: TfrxLineView
          Align = baCenter
          Left = -56.692950000000000000
          Top = 86.929190000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
      end
      object Footer1: TfrxFooter
        FillType = ftBrush
        Height = 185.196970000000000000
        Top = 325.039580000000000000
        Width = 188.976500000000000000
        object Line4: TfrxLineView
          Align = baCenter
          Left = -56.692950000000000000
          Top = 79.370130000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
        object Line5: TfrxLineView
          Align = baCenter
          Left = -56.692950000000000000
          Top = 151.181200000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
        object Memo13: TfrxMemoView
          Align = baLeft
          Top = 58.692950000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Ass: Caixa')
          ParentFont = False
          WordWrap = False
        end
        object Memo14: TfrxMemoView
          Align = baLeft
          Top = 130.504020000000000000
          Width = 83.149660000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Ass: Supervisor')
          ParentFont = False
          WordWrap = False
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Height = 60.472480000000000000
        Top = 241.889920000000000000
        Width = 188.976500000000000000
        DataSet = frxDBdinheiro
        DataSetName = 'frxDBdinheiro'
        RowCount = 0
        object Memo7: TfrxMemoView
          Align = baLeft
          Top = 1.677180000000000000
          Width = 211.653680000000000000
          Height = 15.118110240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDdinheiro.."FORMA_PAGAMENTO"]')
          ParentFont = False
        end
        object Memo8: TfrxMemoView
          Top = 16.795300000000000000
          Width = 86.929190000000000000
          Height = 15.118110240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDdinheiro.."TOTAL"]')
          ParentFont = False
        end
      end
    end
  end
  object frxEmitente: TfrxDBDataset
    UserName = 'frEmitente'
    CloseDataSource = False
    FieldAliases.Strings = (
      'CODIGO=CODIGO'
      'NOME=NOME'
      'FANTASIA=FANTASIA'
      'ENDERECO=ENDERECO'
      'BAIRRO=BAIRRO'
      'CIDADE=CIDADE'
      'UF=UF'
      'CEP=CEP'
      'TELEFONE=TELEFONE'
      'CNPJ=CNPJ'
      'IE=IE'
      'EMAIL=EMAIL'
      'HOMEPAGE=HOMEPAGE'
      'RESPONSAVEL=RESPONSAVEL'
      'CELULAR=CELULAR'
      'LOGO=LOGO'
      'FAX=FAX'
      'NUMERO=NUMERO'
      'COMPLEMENTO=COMPLEMENTO'
      'CONTRIBUINTE_IPI=CONTRIBUINTE_IPI'
      'SUBSTITUTO_TRIBUTARIO=SUBSTITUTO_TRIBUTARIO'
      'COMENTARIOS=COMENTARIOS'
      'DATAHORA_INICIAL=DATAHORA_INICIAL'
      'DATAHORA_FINAL=DATAHORA_FINAL'
      'DATA_INVENTARIO=DATA_INVENTARIO'
      'ESTOQUE=ESTOQUE'
      'VALOR=VALOR'
      'CONHECIMENTO=CONHECIMENTO'
      'CPF=CPF'
      'RG=RG'
      'DIA_VENCIMENTO_CHAVE=DIA_VENCIMENTO_CHAVE'
      'INSC_MUNICIPAL=INSC_MUNICIPAL'
      'DATA_ABERTURA=DATA_ABERTURA'
      'COD_MUNICIPIO_IBGE=COD_MUNICIPIO_IBGE'
      'IBGE=IBGE'
      'EMAIL_FINANCEIRO=EMAIL_FINANCEIRO')
    DataSet = frmModulo.Qremitente
    BCDToCurrency = False
    Left = 480
    Top = 144
  end
  object frxPDFExport1: TfrxPDFExport
    UseFileCache = True
    ShowProgress = True
    OverwritePrompt = False
    DataOnly = False
    PrintOptimized = False
    Outline = False
    Background = False
    HTMLTags = True
    Quality = 95
    Transparency = False
    Author = 'FastReport'
    Subject = 'FastReport PDF export'
    ProtectionFlags = [ePrint, eModify, eCopy, eAnnot]
    HideToolbar = False
    HideMenubar = False
    HideWindowUI = False
    FitWindow = False
    CenterWindow = False
    PrintScaling = False
    PdfA = False
    Left = 216
    Top = 472
  end
  object cdsDados: TClientDataSet
    Aggregates = <>
    FieldDefs = <>
    IndexDefs = <
      item
        Name = 'cdsDadosIndex1'
        Fields = 'Linha'
      end>
    IndexFieldNames = 'Linha'
    Params = <>
    StoreDefs = True
    Left = 605
    Top = 458
    object cdsDadosDescricao: TStringField
      FieldName = 'Descricao'
      Size = 100
    end
    object cdsDadosValor: TStringField
      FieldName = 'Valor'
      Size = 50
    end
    object cdsDadosnegrito: TStringField
      FieldName = 'negrito'
      Size = 1
    end
    object cdsDadoslinha: TIntegerField
      FieldName = 'linha'
    end
  end
  object frxDados: TfrxDBDataset
    UserName = 'frDados'
    CloseDataSource = False
    FieldAliases.Strings = (
      'Descricao=Descricao'
      'Valor=VALOR'
      'negrito=negrito')
    DataSet = cdsDados
    BCDToCurrency = False
    Left = 536
    Top = 472
  end
  object ActionManager1: TActionManager
    Left = 256
    Top = 368
    StyleName = 'Platform Default'
    object Action1: TAction
      Caption = 'Editar'
      ShortCut = 16453
      OnExecute = Action1Execute
    end
  end
  object frxdbdtst1: TfrxDBDataset
    UserName = 'frxDBDataset1'
    CloseDataSource = False
    DataSet = frmModulo.qrcaixa_operador
    BCDToCurrency = False
    Left = 344
    Top = 472
  end
  object frxDBDFechamento: TfrxDBDataset
    UserName = 'frxDBDFechamento'
    CloseDataSource = False
    DataSet = qrFechamento
    BCDToCurrency = False
    Left = 568
    Top = 264
  end
  object frxDBDsuprimento: TfrxDBDataset
    UserName = 'frxDBDsuprimento'
    CloseDataSource = False
    DataSet = qrsuprimento
    BCDToCurrency = False
    Left = 904
    Top = 408
  end
  object qrsuprimento: TUniQuery
    Connection = frmModulo.Conexao_Servidor
    SQL.Strings = (
      'SELECT CODVENDEDOR, VALOR AS VALORS'
      'FROM NAO_FISCAL')
    Left = 933
    Top = 406
  end
  object qrsangria: TUniQuery
    Connection = frmModulo.Conexao_Servidor
    SQL.Strings = (
      'SELECT CODVENDEDOR, VALOR AS VALORS'
      'FROM NAO_FISCAL')
    Left = 933
    Top = 454
  end
  object frxDBDsangria: TfrxDBDataset
    UserName = 'frxDBDsangria'
    CloseDataSource = False
    DataSet = qrsangria
    BCDToCurrency = False
    Left = 904
    Top = 456
  end
  object frxDBdinheiro: TfrxDBDataset
    UserName = 'frxDBdinheiro'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'CODOPERADOR=CODOPERADOR'
      'NOME=NOME'
      'MOVIMENTO=MOVIMENTO'
      'VALOR=VALOR')
    DataSet = qrdinheiro
    BCDToCurrency = False
    Left = 904
    Top = 120
  end
  object frxDBaprazo: TfrxDBDataset
    UserName = 'frxDBaprazo'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'CODOPERADOR=CODOPERADOR'
      'NOME=NOME'
      'MOVIMENTO=MOVIMENTO'
      'VALOR=VALOR')
    DataSet = qraprazo
    BCDToCurrency = False
    Left = 904
    Top = 168
  end
  object qrdinheiro: TUniQuery
    Connection = frmModulo.Conexao_Servidor
    SQL.Strings = (
      
        'select CX.DATA, CX.codoperador, FUN.nome, CX.movimento, SUM(CX.V' +
        'ALOR) AS VALOR'
      'FROM C000044 CX, c000008 FUN'
      'WHERE CX.movimento = 3'
      'group BY CX.data, CX.movimento, CX.codoperador, FUN.nome')
    Left = 933
    Top = 118
  end
  object qraprazo: TUniQuery
    Connection = frmModulo.Conexao_Servidor
    SQL.Strings = (
      
        'select CX.DATA, CX.codoperador, FUN.nome, CX.movimento, SUM(CX.V' +
        'ALOR) AS VALOR'
      'FROM C000044 CX, c000008 FUN'
      'WHERE CX.movimento = 4'
      'group BY CX.data, CX.movimento, CX.codoperador, FUN.nome')
    Left = 933
    Top = 166
  end
  object frxDBcartaoc: TfrxDBDataset
    UserName = 'frxDBcartaoc'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'CODOPERADOR=CODOPERADOR'
      'NOME=NOME'
      'MOVIMENTO=MOVIMENTO'
      'VALOR=VALOR')
    DataSet = qrcartaoc
    BCDToCurrency = False
    Left = 904
    Top = 216
  end
  object frxDBcartaod: TfrxDBDataset
    UserName = 'frxDBcartaod'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'CODOPERADOR=CODOPERADOR'
      'NOME=NOME'
      'MOVIMENTO=MOVIMENTO'
      'VALOR=VALOR')
    DataSet = qrcartaod
    BCDToCurrency = False
    Left = 904
    Top = 264
  end
  object qrcartaoc: TUniQuery
    Connection = frmModulo.Conexao_Servidor
    SQL.Strings = (
      
        'select CX.DATA, CX.codoperador, FUN.nome, CX.movimento, SUM(CX.V' +
        'ALOR) AS VALOR'
      'FROM C000044 CX, c000008 FUN'
      'WHERE CX.movimento = 7'
      'group BY CX.data, CX.movimento, CX.codoperador, FUN.nome')
    Left = 933
    Top = 214
  end
  object qrcartaod: TUniQuery
    Connection = frmModulo.Conexao_Servidor
    SQL.Strings = (
      
        'select CX.DATA, CX.codoperador, FUN.nome, CX.movimento, SUM(CX.V' +
        'ALOR) AS VALOR'
      'FROM C000044 CX, c000008 FUN'
      'WHERE CX.movimento = 8'
      'group BY CX.data, CX.movimento, CX.codoperador, FUN.nome')
    Left = 933
    Top = 262
  end
  object frxrelatorio: TfrxReport
    Version = '5.4.6'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.AllowEdit = False
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Padr'#227'o'
    PrintOptions.PrintOnSheet = 0
    PrintOptions.ShowDialog = False
    ReportOptions.CreateDate = 43652.587404467600000000
    ReportOptions.LastChange = 43984.975033310200000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      '                       '
      'begin'
      '                                                            '
      'end.')
    ShowProgress = False
    Left = 224
    Top = 84
    Datasets = <
      item
        DataSet = frxDados
        DataSetName = 'frDados'
      end
      item
        DataSet = frxEmitente
        DataSetName = 'frEmitente'
      end
      item
        DataSet = frxDBaprazo1
        DataSetName = 'frxDBaprazo1'
      end
      item
        DataSet = frxDBcartaoc1
        DataSetName = 'frxDBcartaoc1'
      end
      item
        DataSet = frxDBcartaod1
        DataSetName = 'frxDBcartaod1'
      end
      item
        DataSet = frxdbdtst1
        DataSetName = 'frxDBDataset1'
      end
      item
        DataSet = frxDBdep_transf1
        DataSetName = 'frxDBdep_transf1'
      end
      item
        DataSet = frxDBDFechamento
        DataSetName = 'frxDBDFechamento'
      end
      item
        DataSet = frxDBdinheiro1
        DataSetName = 'frxDBdinheiro1'
      end
      item
        DataSet = frxDBDsangria1
        DataSetName = 'frxDBDsangria1'
      end
      item
        DataSet = frxDBDsuprimento1
        DataSetName = 'frxDBDsuprimento1'
      end
      item
        DataSet = frxDBpix1
        DataSetName = 'frxDBpix1'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 60.000000000000000000
      PaperHeight = 230.000000000000000000
      PaperSize = 256
      LeftMargin = 5.000000000000000000
      RightMargin = 5.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        Height = 128.740260000000000000
        Top = 18.897650000000000000
        Width = 188.976500000000000000
        object Memo25: TfrxMemoView
          Align = baCenter
          Left = 3.779530000000000000
          Top = 0.220470000000000000
          Width = 181.417440000000000000
          Height = 37.795300000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."FANTASIA"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo29: TfrxMemoView
          Align = baCenter
          Left = 5.669295000000000000
          Top = 25.354360000000000000
          Width = 177.637910000000000000
          Height = 34.015770000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."CNPJ"]'
            ' [frEmitente."IE"]')
          ParentFont = False
          WordWrap = False
          Formats = <
            item
            end
            item
            end>
        end
        object Memo31: TfrxMemoView
          Align = baCenter
          Left = -35.905535000000000000
          Top = 58.149660000000000000
          Width = 260.787570000000000000
          Height = 22.677180000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '*** FECHAMENTO CAIXA ***')
          ParentFont = False
          WordWrap = False
        end
        object Memo32: TfrxMemoView
          Align = baRight
          Left = 71.811070000000000000
          Top = 88.385900000000000000
          Width = 117.165430000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Hor'#225'rio: [Time]')
          ParentFont = False
          WordWrap = False
        end
        object Line9: TfrxLineView
          Align = baCenter
          Left = -56.692950000000000000
          Top = 56.149660000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
        object Line11: TfrxLineView
          Align = baCenter
          Left = -56.692950000000000000
          Top = 124.960730000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
        object Memo26: TfrxMemoView
          Top = 75.590600000000000000
          Width = 166.299320000000000000
          Height = 15.118110240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'DATA..: [frxDBdinheiro1."DATA"]')
          ParentFont = False
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Height = 15.118120000000000000
        Top = 445.984540000000000000
        Width = 188.976500000000000000
        DataSet = frxDBDsangria1
        DataSetName = 'frxDBDsangria1'
        RowCount = 0
        object Memo7: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 15.118110240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '(Saida)[frxDBDsangria1."FORMA_PAGAMENTOS"].-.:')
          ParentFont = False
        end
        object Memo8: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 15.118110240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDsangria1."VALORS"]')
          ParentFont = False
        end
      end
      object MasterData2: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 411.968770000000000000
        Width = 188.976500000000000000
        DataSet = frxDBDsuprimento1
        DataSetName = 'frxDBDsuprimento1'
        RowCount = 0
        object Memo1: TfrxMemoView
          Align = baLeft
          Width = 117.165430000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '(Entrada)[frxDBDsuprimento1."FORMA_PAGAMENTOSP"].+.:')
          ParentFont = False
        end
        object Memo2: TfrxMemoView
          Left = 116.488250000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDsuprimento1."VALORSP"]')
          ParentFont = False
        end
      end
      object MasterData3: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 377.953000000000000000
        Width = 188.976500000000000000
        DataSet = frxDBdinheiro1
        DataSetName = 'frxDBdinheiro1'
        RowCount = 0
        object Memo3: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDdinheiro1."FORMA_PAGAMENTO"].+.:')
          ParentFont = False
        end
        object Memo4: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDBDdinheiro1."TOTAL">-<frxDBDdinheiro."TROCO">)]')
          ParentFont = False
        end
      end
      object MasterData4: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 275.905690000000000000
        Width = 188.976500000000000000
        DataSet = frxDBcartaod1
        DataSetName = 'frxDBcartaod1'
        RowCount = 0
        object Memo5: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDcartaod1."FORMA_PAGAMENTOD"].+.:')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDcartaod1."TOTALD"]')
          ParentFont = False
        end
      end
      object MasterData5: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 241.889920000000000000
        Width = 188.976500000000000000
        DataSet = frxDBcartaoc1
        DataSetName = 'frxDBcartaoc1'
        RowCount = 0
        object Memo9: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDcartaoc1."FORMA_PAGAMENTOC"].+.:')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDcartaoc1."TOTALC"]')
          ParentFont = False
        end
      end
      object MasterData6: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 207.874150000000000000
        Width = 188.976500000000000000
        DataSet = frxDBaprazo1
        DataSetName = 'frxDBaprazo1'
        RowCount = 0
        object Memo11: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDaprazo1."FORMA_PAGAMENTOAP"].+.:')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDaprazo1."TOTALAP"]')
          ParentFont = False
        end
        object Line2: TfrxLineView
          Left = -49.795300000000000000
          Width = 347.716760000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        Height = 268.346630000000000000
        Top = 521.575140000000000000
        Width = 188.976500000000000000
        object Memo13: TfrxMemoView
          Left = 22.102350000000000000
          Top = 12.338590000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            
              '[(<frxDBDaprazo1."TOTALAP">+<frxDBDcartaoc1."TOTALC">+<frxDBDcar' +
              'taod1."TOTALD">+<frxDBDdinheiro1."TOTAL">+SUM(<frxDBDsuprimento1' +
              '."VALORSP">,MasterData2)-<frxDBDdinheiro1."TROCO">)]')
          ParentFont = False
        end
        object Memo14: TfrxMemoView
          Width = 185.196970000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Movimento Geral_Entrada..: +')
          ParentFont = False
        end
        object Memo15: TfrxMemoView
          Left = 22.440940000000000000
          Top = 44.236240000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            
              '+ [(<frxDBDdinheiro1."TOTAL">+SUM(<frxDBDsuprimento1.VALORSP">,M' +
              'asterData2)-<frxDBDdinheiro1."TROCO">)]')
          ParentFont = False
        end
        object Memo16: TfrxMemoView
          Top = 31.338590000000000000
          Width = 177.637910000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'TOTAL Dinheiro/Suprimento..: ')
          ParentFont = False
        end
        object Line3: TfrxLineView
          Left = -30.236240000000000000
          Top = 26.677180000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line4: TfrxLineView
          Left = -30.236240000000000000
          Top = 66.133890000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line5: TfrxLineView
          Left = -30.236240000000000000
          Top = 121.811070000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo20: TfrxMemoView
          Left = 0.779530000000000000
          Top = 94.149660000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'TOTAL SANGRIA..: ')
          ParentFont = False
        end
        object Memo21: TfrxMemoView
          Left = 26.267780000000000000
          Top = 134.826840000000000000
          Width = 147.401670000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            
              '[(<frxDBDdinheiro1."TOTAL">+SUM(<frxDBDsuprimento1."VALORSP">,Ma' +
              'sterData2)-SUM(<frxDBDsangria1."VALORS">,MasterData1)-<frxDBDdin' +
              'heiro1."TROCO">)]')
          ParentFont = False
        end
        object Memo22: TfrxMemoView
          Left = 0.779530000000000000
          Top = 120.826840000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Saldo em Caixa...: ')
          ParentFont = False
        end
        object Line7: TfrxLineView
          Left = -29.456710000000000000
          Top = 151.740260000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo17: TfrxMemoView
          Left = -1.000000000000000000
          Top = 150.181200000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'Operador do caixa')
          ParentFont = False
        end
        object Line8: TfrxLineView
          Left = -26.456710000000000000
          Top = 177.637910000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo18: TfrxMemoView
          Left = 3.000000000000000000
          Top = 176.078850000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Gerente')
          ParentFont = False
        end
        object Memo23: TfrxMemoView
          Top = 204.094620000000000000
          Width = 188.976500000000000000
          Height = 64.252010000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Observa'#231#227'o:'
            '.'
            '.'
            '.'
            '.')
          ParentFont = False
        end
        object Line10: TfrxLineView
          Left = -55.779530000000000000
          Top = 200.315090000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line12: TfrxLineView
          Left = -76.897650000000000000
          Top = 0.220469999999977000
          Width = 476.220780000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo24: TfrxMemoView
          Top = 67.913420000000000000
          Width = 177.637910000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'TOTAL Suprimento..: ')
          ParentFont = False
        end
        object Line1: TfrxLineView
          Left = -30.236240000000000000
          Top = 94.708720000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object SysMemo1: TfrxSysMemoView
          Left = 60.252010000000000000
          Top = 78.811070000000000000
          Width = 113.385900000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '+[SUM(<frxDBDsuprimento1."VALORSP">,MasterData2)]')
          ParentFont = False
        end
        object SysMemo2: TfrxSysMemoView
          Left = 65.149660000000000000
          Top = 105.047310000000000000
          Width = 109.606370000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '-[SUM(<frxDBDsangria1."VALORS">,MasterData1)]')
          ParentFont = False
        end
      end
      object MasterData7: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 343.937230000000000000
        Width = 188.976500000000000000
        DataSet = frxDBdep_transf1
        DataSetName = 'frxDBdep_transf1'
        RowCount = 0
        object Memo27: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDdeposito_tranf1."FORMA_PAGAMENTO"].+.:')
          ParentFont = False
        end
        object Memo28: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            
              '[(<frxDBDdeposito_tranf1."TOTAL">-<frxDBDdeposito_tranf."TROCO">' +
              ')]')
          ParentFont = False
        end
      end
      object MasterData8: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 309.921460000000000000
        Width = 188.976500000000000000
        DataSet = frxDBpix1
        DataSetName = 'frxDBpix1'
        RowCount = 0
        object Memo30: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDpix1."FORMA_PAGAMENTO"].+.:')
          ParentFont = False
        end
        object Memo33: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDBDpix1."TOTAL">-<frxDBDpix."TROCO">)]')
          ParentFont = False
        end
      end
    end
  end
  object frxRelatorioDetalhado: TfrxReport
    Version = '5.4.6'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Padr'#227'o'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 43652.587404467590000000
    ReportOptions.LastChange = 43652.587404467590000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 592
    Top = 140
    Datasets = <
      item
        DataSet = frxDados
        DataSetName = 'frDados'
      end
      item
        DataSet = frxEmitente
        DataSetName = 'frEmitente'
      end
      item
        DataSet = frxDBaprazo
        DataSetName = 'frxDBaprazo'
      end
      item
        DataSet = frxdbdtst1
        DataSetName = 'frxDBDataset1'
      end
      item
        DataSet = frxDBcartaoc
        DataSetName = 'frxDBcartaoc'
      end
      item
        DataSet = frxDBcartaod
        DataSetName = 'frxDBcartaod'
      end
      item
        DataSet = frxDBdinheiro
        DataSetName = 'frxDBdinheiro'
      end
      item
        DataSet = frxDBDFechamento
        DataSetName = 'frxDBDFechamento'
      end
      item
        DataSet = frxDBDsangria
        DataSetName = 'frxDBDsangria'
      end
      item
        DataSet = frxDBDsuprimento
        DataSetName = 'frxDBDsuprimento'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 60.000000000000000000
      PaperHeight = 230.000000000000000000
      PaperSize = 256
      LeftMargin = 5.000000000000000000
      RightMargin = 5.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        Height = 158.740260000000000000
        Top = 18.897650000000000000
        Width = 188.976500000000000000
        object Memo25: TfrxMemoView
          Align = baCenter
          Left = 68.031540000000000000
          Top = 1.220470000000000000
          Width = 52.913420000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."NOME"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo26: TfrxMemoView
          Align = baCenter
          Left = 71.811070000000000000
          Top = 20.118120000000000000
          Width = 45.354360000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[<frEmitente."ENDERECO">]')
          ParentFont = False
          WordWrap = False
          Formats = <
            item
            end
            item
            end>
        end
        object Memo27: TfrxMemoView
          Align = baCenter
          Left = 47.244125000000000000
          Top = 35.236240000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."CIDADE"] / [frEmitente."UF"] - [frEmitente."CEP"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo28: TfrxMemoView
          Align = baCenter
          Left = 47.244125000000000000
          Top = 50.354360000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."TELEFONE"]')
          ParentFont = False
          WordWrap = False
          Formats = <
            item
            end
            item
            end
            item
            end>
        end
        object Memo29: TfrxMemoView
          Align = baCenter
          Left = 66.141775000000000000
          Top = 65.472480000000000000
          Width = 56.692950000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."CNPJ"] - [frEmitente."IE"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo30: TfrxMemoView
          Align = baLeft
          Top = 118.385900000000000000
          Width = 117.165430000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Data: [Date]')
          ParentFont = False
          WordWrap = False
        end
        object mCaixa: TfrxMemoView
          Left = 2.031540000000000000
          Top = 138.063080000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Caixa: 001')
          ParentFont = False
          WordWrap = False
          Formats = <
            item
            end
            item
            end>
        end
        object mOperador: TfrxMemoView
          Left = 83.291435000000000000
          Top = 138.063080000000000000
          Width = 102.047310000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Operador: [frxDBDataset1."CODFUNCIONARIO"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo31: TfrxMemoView
          Align = baCenter
          Left = -35.905535000000000000
          Top = 88.149660000000000000
          Width = 260.787570000000000000
          Height = 22.677180000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '*** FECHAMENTO CAIXA ***')
          ParentFont = False
          WordWrap = False
        end
        object Memo32: TfrxMemoView
          Align = baRight
          Left = 71.811070000000000000
          Top = 114.606370000000000000
          Width = 117.165430000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Hor'#225'rio: [Time]')
          ParentFont = False
          WordWrap = False
        end
        object Line9: TfrxLineView
          Align = baCenter
          Left = -56.692950000000000000
          Top = 88.149660000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
        object Line11: TfrxLineView
          Align = baCenter
          Left = -56.692950000000000000
          Top = 155.960730000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Height = 15.118120000000000000
        Top = 408.189240000000000000
        Width = 188.976500000000000000
        DataSet = frxDBDsangria
        DataSetName = 'frxDBDsangria'
        RowCount = 0
        object Memo7: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDsangria."FORMA_PAGAMENTOS"].-.:')
          ParentFont = False
        end
        object Memo8: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDsangria."VALORS"]')
          ParentFont = False
        end
      end
      object MasterData2: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 374.173470000000000000
        Width = 188.976500000000000000
        DataSet = frxDBDsuprimento
        DataSetName = 'frxDBDsuprimento'
        RowCount = 0
        object Memo1: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDsuprimento."FORMA_PAGAMENTOSP"].+.:')
          ParentFont = False
        end
        object Memo2: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDsuprimento."VALORSP"]')
          ParentFont = False
        end
      end
      object MasterData3: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 340.157700000000000000
        Width = 188.976500000000000000
        DataSet = frxDBdinheiro
        DataSetName = 'frxDBdinheiro'
        RowCount = 0
        object Memo3: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDdinheiro."FORMA_PAGAMENTO"].+.:')
          ParentFont = False
        end
        object Memo4: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDBDdinheiro."TOTAL">-<frxDBDdinheiro."TROCO">)]')
          ParentFont = False
        end
      end
      object MasterData4: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 306.141930000000000000
        Width = 188.976500000000000000
        DataSet = frxDBcartaod
        DataSetName = 'frxDBcartaod'
        RowCount = 0
        object Memo5: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDcartaod."FORMA_PAGAMENTOD"].+.:')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDcartaod."TOTALD"]')
          ParentFont = False
        end
      end
      object MasterData5: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 272.126160000000000000
        Width = 188.976500000000000000
        DataSet = frxDBcartaoc
        DataSetName = 'frxDBcartaoc'
        RowCount = 0
        object Memo9: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDcartaoc."FORMA_PAGAMENTOC"].+.:')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDcartaoc."TOTALC"]')
          ParentFont = False
        end
      end
      object MasterData6: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 238.110390000000000000
        Width = 188.976500000000000000
        DataSet = frxDBaprazo
        DataSetName = 'frxDBaprazo'
        RowCount = 0
        object Memo11: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDaprazo."FORMA_PAGAMENTOAP"].+.:')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDaprazo."TOTALAP"]')
          ParentFont = False
        end
        object Line2: TfrxLineView
          Left = -49.795300000000000000
          Width = 347.716760000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        Height = 268.346630000000000000
        Top = 483.779840000000000000
        Width = 188.976500000000000000
        object Line1: TfrxLineView
          Left = -64.252010000000000000
          Top = 1.779530000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo13: TfrxMemoView
          Left = 32.102350000000000000
          Top = 12.338590000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            
              '[SUM(<frxDBDaprazo."TOTALAP">+<frxDBDcartaoc."TOTALC">+<frxDBDca' +
              'rtaod."TOTALD">+<frxDBDdinheiro."TOTAL">+<frxDBDsuprimento."VALO' +
              'RSP">+<frxDBDsangria."VALORS">-<frxDBDdinheiro."TROCO">)]')
          ParentFont = False
        end
        object Memo14: TfrxMemoView
          Width = 185.196970000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Movimento Geral_Entrada/Saida..: +')
          ParentFont = False
        end
        object Memo15: TfrxMemoView
          Left = 31.440940000000000000
          Top = 38.236240000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            
              '+ [SUM(<frxDBDdinheiro."TOTAL">+<frxDBDsuprimento."VALORSP">-<fr' +
              'xDBDdinheiro."TROCO">)]')
          ParentFont = False
        end
        object Memo16: TfrxMemoView
          Top = 25.338590000000000000
          Width = 177.637910000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'TOTAL Dinheiro/Suprimento..: ')
          ParentFont = False
        end
        object Line3: TfrxLineView
          Left = -30.236240000000000000
          Top = 26.677180000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line4: TfrxLineView
          Left = -30.236240000000000000
          Top = 52.133890000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line5: TfrxLineView
          Left = -30.236240000000000000
          Top = 77.811070000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo19: TfrxMemoView
          Left = 31.220470000000000000
          Top = 64.047310000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '- [SUM(<frxDBDsangria."VALORS">)]')
          ParentFont = False
        end
        object Memo20: TfrxMemoView
          Left = 0.779530000000000000
          Top = 52.149660000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'TOTAL SANGRIA..: ')
          ParentFont = False
        end
        object Memo21: TfrxMemoView
          Left = 34.267780000000000000
          Top = 90.826840000000000000
          Width = 147.401670000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            
              '[SUM(<frxDBDdinheiro."TOTAL">+<frxDBDsuprimento."VALORSP">-<frxD' +
              'BDsangria."VALORS">-<frxDBDdinheiro."TROCO">)]')
          ParentFont = False
        end
        object Memo22: TfrxMemoView
          Left = 0.779530000000000000
          Top = 78.826840000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Saldo em Caixa...: ')
          ParentFont = False
        end
        object Line6: TfrxLineView
          Left = -29.456710000000000000
          Top = 104.944960000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line7: TfrxLineView
          Left = -29.456710000000000000
          Top = 146.740260000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo17: TfrxMemoView
          Left = -1.000000000000000000
          Top = 146.181200000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'Operador do caixa')
          ParentFont = False
        end
        object Line8: TfrxLineView
          Left = -26.456710000000000000
          Top = 177.637910000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo18: TfrxMemoView
          Left = 3.000000000000000000
          Top = 176.078850000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Gerente')
          ParentFont = False
        end
        object Memo23: TfrxMemoView
          Top = 204.094620000000000000
          Width = 188.976500000000000000
          Height = 64.252010000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Observa'#231#227'o:'
            '.'
            '.'
            '.'
            '.'
            '')
          ParentFont = False
        end
        object Line10: TfrxLineView
          Left = -55.779530000000000000
          Top = 200.315090000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
      end
    end
  end
  object frxdad: TfrxDBDataset
    UserName = 'frxdad'
    CloseDataSource = False
    DataSet = qrdados
    BCDToCurrency = False
    Left = 488
    Top = 224
  end
  object qrdados: TUniQuery
    Connection = frmModulo.conexao
    Left = 304
    Top = 296
  end
  object frxRel_Ven_Canceladas: TfrxReport
    Version = '5.4.6'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.AllowEdit = False
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Padr'#227'o'
    PrintOptions.PrintOnSheet = 0
    PrintOptions.ShowDialog = False
    ReportOptions.CreateDate = 43927.878265034700000000
    ReportOptions.LastChange = 43927.878265034700000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    ShowProgress = False
    Left = 392
    Top = 228
    Datasets = <
      item
        DataSet = frxEmitente
        DataSetName = 'frEmitente'
      end
      item
        DataSet = frxdbdtst1
        DataSetName = 'frxDBDataset1'
      end
      item
        DataSet = frxDBdinheiro
        DataSetName = 'frxDBdinheiro'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 74.000000000000000000
      PaperHeight = 306.000000000000000000
      PaperSize = 256
      LeftMargin = 5.000000000000000000
      RightMargin = 5.000000000000000000
      TopMargin = 5.000000000000000000
      BottomMargin = 5.000000000000000000
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = []
        Height = 181.417440000000000000
        ParentFont = False
        Top = 18.897650000000000000
        Width = 241.889920000000000000
        object Memo2: TfrxMemoView
          Align = baCenter
          Left = 94.488250000000000000
          Width = 52.913420000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."FANTASIA"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo1: TfrxMemoView
          Align = baCenter
          Left = 98.267780000000000000
          Top = 18.897650000000000000
          Width = 45.354360000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[<frEmitente."ENDERECO">], [<frEmitente."NUMERO">]')
          ParentFont = False
          WordWrap = False
        end
        object Memo3: TfrxMemoView
          Align = baCenter
          Left = 73.700835000000000000
          Top = 34.015770000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."CIDADE"] / [frEmitente."UF"] - [frEmitente."CEP"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo4: TfrxMemoView
          Align = baCenter
          Left = 73.700835000000000000
          Top = 49.133890000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."TELEFONE"]')
          ParentFont = False
          WordWrap = False
          Formats = <
            item
            end
            item
            end
            item
            end>
        end
        object Memo5: TfrxMemoView
          Align = baCenter
          Left = 92.598485000000000000
          Top = 64.252010000000000000
          Width = 56.692950000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."CNPJ"] - [frEmitente."IE"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo6: TfrxMemoView
          Align = baLeft
          Left = 121.472480000000000000
          Top = 117.165430000000000000
          Width = 117.165430000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Data: [Date]')
          ParentFont = False
          WordWrap = False
        end
        object mCaixa: TfrxMemoView
          Top = 139.842610000000000000
          Width = 37.795300000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Caixa:')
          ParentFont = False
          WordWrap = False
        end
        object mOperador: TfrxMemoView
          Left = 39.685065000000000000
          Top = 139.842610000000000000
          Width = 102.047310000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDataset1."CODFUNCIONARIO"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo9: TfrxMemoView
          Align = baCenter
          Left = -9.448825000000000000
          Top = 86.929190000000000000
          Width = 260.787570000000000000
          Height = 22.677180000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '*** VENDAS CANCELADAS ***')
          ParentFont = False
          WordWrap = False
        end
        object Memo10: TfrxMemoView
          Align = baRight
          Left = 4.307050000000000000
          Top = 113.385900000000000000
          Width = 117.165430000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Hor'#225'rio: [Time]')
          ParentFont = False
          WordWrap = False
        end
        object Line1: TfrxLineView
          Align = baCenter
          Left = -30.236240000000000000
          Top = 86.929190000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
        object Line3: TfrxLineView
          Align = baCenter
          Left = -30.236240000000000000
          Top = 156.078850000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
        object Memo11: TfrxMemoView
          Left = 7.559060000000000000
          Top = 158.519790000000000000
          Width = 234.330860000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'N'#186' Cupom:      Horas:             Valor:')
          ParentFont = False
        end
        object Line2: TfrxLineView
          Align = baCenter
          Left = -30.236240000000000000
          Top = 179.417440000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Height = 17.574820240000000000
        Top = 260.787570000000000000
        Width = 241.889920000000000000
        DataSet = frxDBdinheiro
        DataSetName = 'frxDBdinheiro'
        RowCount = 0
        object Memo7: TfrxMemoView
          Align = baBottom
          Left = 8.338590000000000000
          Top = 2.456710000000000000
          Width = 68.031540000000000000
          Height = 15.118110240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haBlock
          Memo.UTF8W = (
            '[frxDBDdinheiro."NUMERO"]  =>')
          ParentFont = False
        end
        object Memo8: TfrxMemoView
          Left = 178.315090000000000000
          Top = 2.456710000000000000
          Width = 49.133890000000000000
          Height = 15.118110240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDdinheiro."VALOR_TOTAL"]')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Align = baBottom
          Left = 83.149660000000000000
          Top = 2.456710000000000000
          Width = 79.370130000000000000
          Height = 15.118110240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haBlock
          Memo.UTF8W = (
            '[frxDBDdinheiro."HORA"]  =>')
          ParentFont = False
        end
      end
    end
  end
  object qrconfig: TUniQuery
    Connection = frmModulo.conexao
    SQL.Strings = (
      'select * from config')
    Left = 372
    Top = 115
  end
  object frxrelatorio4: TfrxReport
    Version = '5.4.6'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Padr'#227'o'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 43652.587404467590000000
    ReportOptions.LastChange = 43652.587404467590000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 464
    Top = 52
    Datasets = <
      item
        DataSet = frxDados
        DataSetName = 'frDados'
      end
      item
        DataSet = frxEmitente
        DataSetName = 'frEmitente'
      end
      item
        DataSet = frxDBaprazo
        DataSetName = 'frxDBaprazo'
      end
      item
        DataSet = frxdbdtst1
        DataSetName = 'frxDBDataset1'
      end
      item
        DataSet = frxDBcartaoc
        DataSetName = 'frxDBcartaoc'
      end
      item
        DataSet = frxDBcartaod
        DataSetName = 'frxDBcartaod'
      end
      item
        DataSet = frxDBdinheiro
        DataSetName = 'frxDBdinheiro'
      end
      item
        DataSet = frxDBDFechamento
        DataSetName = 'frxDBDFechamento'
      end
      item
        DataSet = frxDBDsangria
        DataSetName = 'frxDBDsangria'
      end
      item
        DataSet = frxDBDsuprimento
        DataSetName = 'frxDBDsuprimento'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 60.000000000000000000
      PaperHeight = 230.000000000000000000
      PaperSize = 256
      LeftMargin = 5.000000000000000000
      RightMargin = 5.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        Height = 128.740260000000000000
        Top = 18.897650000000000000
        Width = 188.976500000000000000
        object Memo25: TfrxMemoView
          Align = baCenter
          Left = 3.779530000000000000
          Top = 9.559060000000000000
          Width = 181.417440000000000000
          Height = 37.795300000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."FANTASIA"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo29: TfrxMemoView
          Align = baCenter
          Left = 5.669295000000000000
          Top = 25.354360000000000000
          Width = 177.637910000000000000
          Height = 34.015770000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."CNPJ"]'
            ' [frEmitente."IE"]')
          ParentFont = False
          WordWrap = False
          Formats = <
            item
            end
            item
            end>
        end
        object Memo30: TfrxMemoView
          Align = baLeft
          Top = 85.385900000000000000
          Width = 117.165430000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Data: [Date]')
          ParentFont = False
          WordWrap = False
        end
        object mCaixa: TfrxMemoView
          Left = 2.031540000000000000
          Top = 108.063080000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Caixa: [frxDBDataset1."CODFUNCIONARIO"] /')
          ParentFont = False
          WordWrap = False
        end
        object mOperador: TfrxMemoView
          Left = 83.291435000000000000
          Top = 108.063080000000000000
          Width = 102.047310000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Operador: [frxDBDataset1."CODFUNCIONARIO"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo31: TfrxMemoView
          Align = baCenter
          Left = -35.905535000000000000
          Top = 58.149660000000000000
          Width = 260.787570000000000000
          Height = 22.677180000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '*** FECHAMENTO CAIXA ***')
          ParentFont = False
          WordWrap = False
        end
        object Memo32: TfrxMemoView
          Align = baRight
          Left = 71.811070000000000000
          Top = 84.606370000000000000
          Width = 117.165430000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Hor'#225'rio: [Time]')
          ParentFont = False
          WordWrap = False
        end
        object Line9: TfrxLineView
          Align = baCenter
          Left = -56.692950000000000000
          Top = 56.149660000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
        object Line11: TfrxLineView
          Align = baCenter
          Left = -56.692950000000000000
          Top = 124.960730000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Height = 15.118120000000000000
        Top = 377.953000000000000000
        Width = 188.976500000000000000
        DataSet = frxDBDsangria
        DataSetName = 'frxDBDsangria'
        RowCount = 0
        object Memo7: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 15.118110240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '(Saida)[frxDBDsangria."FORMA_PAGAMENTOS"].-.:')
          ParentFont = False
        end
        object Memo8: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 15.118110240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDsangria."VALORS"]')
          ParentFont = False
        end
      end
      object MasterData2: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 343.937230000000000000
        Width = 188.976500000000000000
        DataSet = frxDBDsuprimento
        DataSetName = 'frxDBDsuprimento'
        RowCount = 0
        object Memo1: TfrxMemoView
          Align = baLeft
          Width = 117.165430000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '(Entrada)[frxDBDsuprimento."FORMA_PAGAMENTOSP"].+.:')
          ParentFont = False
        end
        object Memo2: TfrxMemoView
          Left = 116.488250000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDsuprimento."VALORSP"]')
          ParentFont = False
        end
      end
      object MasterData3: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 309.921460000000000000
        Width = 188.976500000000000000
        DataSet = frxDBdinheiro
        DataSetName = 'frxDBdinheiro'
        RowCount = 0
        object Memo3: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDdinheiro."FORMA_PAGAMENTO"].+.:')
          ParentFont = False
        end
        object Memo4: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDBDdinheiro."TOTAL">-<frxDBDdinheiro."TROCO">)]')
          ParentFont = False
        end
      end
      object MasterData4: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 275.905690000000000000
        Width = 188.976500000000000000
        DataSet = frxDBcartaod
        DataSetName = 'frxDBcartaod'
        RowCount = 0
        object Memo5: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDcartaod."FORMA_PAGAMENTOD"].+.:')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDcartaod."TOTALD"]')
          ParentFont = False
        end
      end
      object MasterData5: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 241.889920000000000000
        Width = 188.976500000000000000
        DataSet = frxDBcartaoc
        DataSetName = 'frxDBcartaoc'
        RowCount = 0
        object Memo9: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDcartaoc."FORMA_PAGAMENTOC"].+.:')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDcartaoc."TOTALC"]')
          ParentFont = False
        end
      end
      object MasterData6: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 207.874150000000000000
        Width = 188.976500000000000000
        DataSet = frxDBaprazo
        DataSetName = 'frxDBaprazo'
        RowCount = 0
        object Memo11: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDaprazo."FORMA_PAGAMENTOAP"].+.:')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Left = 101.370130000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDaprazo."TOTALAP"]')
          ParentFont = False
        end
        object Line2: TfrxLineView
          Left = -49.795300000000000000
          Width = 347.716760000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        Height = 268.346630000000000000
        Top = 453.543600000000000000
        Width = 188.976500000000000000
        object Memo13: TfrxMemoView
          Left = 22.102350000000000000
          Top = 12.338590000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            
              '[(<frxDBDaprazo."TOTALAP">+<frxDBDcartaoc."TOTALC">+<frxDBDcarta' +
              'od."TOTALD">+<frxDBDdinheiro."TOTAL">+SUM(<frxDBDsuprimento."VAL' +
              'ORSP">,MasterData2)-<frxDBDdinheiro."TROCO">)]')
          ParentFont = False
        end
        object Memo14: TfrxMemoView
          Width = 185.196970000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Movimento Geral_Entrada..: +')
          ParentFont = False
        end
        object Memo15: TfrxMemoView
          Left = 22.440940000000000000
          Top = 44.236240000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            
              '+ [(<frxDBDdinheiro."TOTAL">+SUM(<frxDBDsuprimento."VALORSP">,Ma' +
              'sterData2)-<frxDBDdinheiro."TROCO">)]')
          ParentFont = False
        end
        object Memo16: TfrxMemoView
          Top = 31.338590000000000000
          Width = 177.637910000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'TOTAL Dinheiro/Suprimento..: ')
          ParentFont = False
        end
        object Line3: TfrxLineView
          Left = -30.236240000000000000
          Top = 26.677179999999960000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line4: TfrxLineView
          Left = -30.236240000000000000
          Top = 66.133890000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line5: TfrxLineView
          Left = -30.236240000000000000
          Top = 121.811070000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo20: TfrxMemoView
          Left = 0.779530000000000000
          Top = 94.149660000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'TOTAL SANGRIA..: ')
          ParentFont = False
        end
        object Memo21: TfrxMemoView
          Left = 26.267780000000000000
          Top = 134.826840000000000000
          Width = 147.401670000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            
              '[(<frxDBDdinheiro."TOTAL">+SUM(<frxDBDsuprimento."VALORSP">,Mast' +
              'erData2)-SUM(<frxDBDsangria."VALORS">,MasterData1)-<frxDBDdinhei' +
              'ro."TROCO">)]')
          ParentFont = False
        end
        object Memo22: TfrxMemoView
          Left = 0.779530000000000000
          Top = 120.826840000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Saldo em Caixa...: ')
          ParentFont = False
        end
        object Line7: TfrxLineView
          Left = -29.456710000000000000
          Top = 151.740260000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo17: TfrxMemoView
          Left = -1.000000000000000000
          Top = 150.181200000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'Operador do caixa')
          ParentFont = False
        end
        object Line8: TfrxLineView
          Left = -26.456710000000000000
          Top = 177.637910000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo18: TfrxMemoView
          Left = 3.000000000000000000
          Top = 176.078850000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Gerente')
          ParentFont = False
        end
        object Memo23: TfrxMemoView
          Top = 204.094620000000000000
          Width = 188.976500000000000000
          Height = 64.252010000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Observa'#231#227'o:'
            '.'
            '.'
            '.'
            '.'
            '')
          ParentFont = False
        end
        object Line10: TfrxLineView
          Left = -55.779530000000000000
          Top = 200.315090000000100000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line12: TfrxLineView
          Left = -76.897650000000000000
          Top = 0.220469999999977500
          Width = 476.220780000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo24: TfrxMemoView
          Top = 67.913420000000000000
          Width = 177.637910000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'TOTAL Suprimento..: ')
          ParentFont = False
        end
        object Line1: TfrxLineView
          Left = -30.236240000000000000
          Top = 94.708720000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object SysMemo1: TfrxSysMemoView
          Left = 60.252010000000000000
          Top = 78.811070000000000000
          Width = 113.385900000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '+[SUM(<frxDBDsuprimento."VALORSP">,MasterData2)]')
          ParentFont = False
        end
        object SysMemo2: TfrxSysMemoView
          Left = 65.149660000000000000
          Top = 105.047310000000000000
          Width = 109.606370000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '-[SUM(<frxDBDsangria."VALORS">,MasterData1)]')
          ParentFont = False
        end
      end
    end
  end
  object frxDBpix: TfrxDBDataset
    UserName = 'frxDBpix'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'CODOPERADOR=CODOPERADOR'
      'NOME=NOME'
      'MOVIMENTO=MOVIMENTO'
      'VALOR=VALOR')
    DataSet = qrpix
    BCDToCurrency = False
    Left = 904
    Top = 312
  end
  object frxDBdep_transf: TfrxDBDataset
    UserName = 'frxDBdep_transf'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'CODOPERADOR=CODOPERADOR'
      'NOME=NOME'
      'MOVIMENTO=MOVIMENTO'
      'VALOR=VALOR')
    DataSet = qrdep_transf
    BCDToCurrency = False
    Left = 904
    Top = 360
  end
  object qrpix: TUniQuery
    Connection = frmModulo.Conexao_Servidor
    SQL.Strings = (
      
        'select CX.DATA, CX.codoperador, FUN.nome, CX.movimento, SUM(CX.V' +
        'ALOR) AS VALOR'
      'FROM C000044 CX, c000008 FUN'
      'WHERE CX.movimento = 5'
      'group BY CX.data, CX.movimento, CX.codoperador, FUN.nome')
    Left = 933
    Top = 310
  end
  object qrdep_transf: TUniQuery
    Connection = frmModulo.Conexao_Servidor
    SQL.Strings = (
      
        'select CX.DATA, CX.codoperador, FUN.nome, CX.movimento, SUM(CX.V' +
        'ALOR) AS VALOR'
      'FROM C000044 CX, c000008 FUN'
      'WHERE CX.movimento = 6'
      'group BY CX.data, CX.movimento, CX.codoperador, FUN.nome')
    Left = 933
    Top = 358
  end
  object frxRelSimples: TfrxReport
    Version = '5.4.6'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.AllowEdit = False
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Padr'#227'o'
    PrintOptions.PrintOnSheet = 0
    PrintOptions.ShowDialog = False
    ReportOptions.CreateDate = 43652.587404467600000000
    ReportOptions.LastChange = 44614.876760405100000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      '                       '
      'begin'
      '                                                            '
      'end.')
    ShowProgress = False
    Left = 784
    Top = 44
    Datasets = <
      item
        DataSet = frxDados
        DataSetName = 'frDados'
      end
      item
        DataSet = frxEmitente
        DataSetName = 'frEmitente'
      end
      item
        DataSet = frxcodcx
        DataSetName = 'frxcodcx'
      end
      item
        DataSet = frxDBaprazo
        DataSetName = 'frxDBaprazo'
      end
      item
        DataSet = frxDBcartaoc
        DataSetName = 'frxDBcartaoc'
      end
      item
        DataSet = frxDBcartaod
        DataSetName = 'frxDBcartaod'
      end
      item
        DataSet = frxDBdep_transf
        DataSetName = 'frxDBdep_transf'
      end
      item
        DataSet = frxDBDFechamento
        DataSetName = 'frxDBDFechamento'
      end
      item
        DataSet = frxDBdinheiro
        DataSetName = 'frxDBdinheiro'
      end
      item
        DataSet = frxDBDsangria
        DataSetName = 'frxDBDsangria'
      end
      item
        DataSet = frxDBDsuprimento
        DataSetName = 'frxDBDsuprimento'
      end
      item
        DataSet = frxDBpix
        DataSetName = 'frxDBpix'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 60.000000000000000000
      PaperHeight = 230.000000000000000000
      PaperSize = 256
      LeftMargin = 5.000000000000000000
      RightMargin = 5.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        Height = 143.858380000000000000
        Top = 18.897650000000000000
        Width = 188.976500000000000000
        object Memo25: TfrxMemoView
          Align = baCenter
          Left = 3.779530000000000000
          Top = 0.220470000000000000
          Width = 181.417440000000000000
          Height = 37.795300000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."FANTASIA"]')
          ParentFont = False
          WordWrap = False
        end
        object Memo29: TfrxMemoView
          Align = baCenter
          Left = 5.669295000000000000
          Top = 19.354360000000000000
          Width = 177.637910000000000000
          Height = 26.456710000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frEmitente."CNPJ"]'
            ' [frEmitente."IE"]')
          ParentFont = False
          WordWrap = False
          Formats = <
            item
            end
            item
            end>
        end
        object Memo31: TfrxMemoView
          Align = baCenter
          Left = -35.905535000000000000
          Top = 49.149660000000000000
          Width = 260.787570000000000000
          Height = 22.677180000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '*** FECHAMENTO CAIXA ***')
          ParentFont = False
          WordWrap = False
        end
        object Memo32: TfrxMemoView
          Align = baRight
          Left = 3.779530000000000000
          Top = 80.385900000000000000
          Width = 185.196970000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Hor'#225'rio: [Time]')
          ParentFont = False
          WordWrap = False
        end
        object Line9: TfrxLineView
          Align = baCenter
          Left = -56.692950000000000000
          Top = 49.149660000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
        object Line11: TfrxLineView
          Align = baCenter
          Left = -56.692950000000000000
          Top = 141.960730000000000000
          Width = 302.362400000000000000
          Color = clBlack
          Frame.Style = fsDot
          Diagonal = True
        end
        object Memo19: TfrxMemoView
          Left = 1.677180000000000000
          Top = 117.606370000000000000
          Width = 185.196970000000000000
          Height = 22.677170240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Operador..: [frxDBdinheiro."NOME"]')
          ParentFont = False
        end
        object Memo26: TfrxMemoView
          Top = 68.590600000000000000
          Width = 185.196970000000000000
          Height = 15.118110240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            'DATA..: [frxDBdinheiro."DATA"]')
          ParentFont = False
        end
        object Memo34: TfrxMemoView
          Left = 3.779530000000000000
          Top = 102.944960000000000000
          Width = 37.795300000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Caixa:')
          ParentFont = False
        end
        object frxcodcxCOD_CAIXA: TfrxMemoView
          Left = 41.692950000000000000
          Top = 102.181200000000000000
          Width = 79.370130000000000000
          Height = 15.118120000000000000
          DataSet = frxcodcx
          DataSetName = 'frxcodcx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxcodcx."COD_CAIXA"]')
          ParentFont = False
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Height = 15.118120000000000000
        Top = 461.102660000000000000
        Width = 188.976500000000000000
        DataSet = frxDBDsangria
        DataSetName = 'frxDBDsangria'
        RowCount = 0
        object Memo7: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 15.118110240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '(Saida)Sangria.-.:')
          ParentFont = False
        end
        object Memo8: TfrxMemoView
          Align = baLeft
          Left = 102.047310000000000000
          Width = 86.929190000000000000
          Height = 15.118110240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDsangria."VALORS"]')
          ParentFont = False
        end
      end
      object MasterData2: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 427.086890000000000000
        Width = 188.976500000000000000
        DataSet = frxDBDsuprimento
        DataSetName = 'frxDBDsuprimento'
        RowCount = 0
        object Memo1: TfrxMemoView
          Align = baLeft
          Width = 117.165430000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '(Entrada)Suprimento.+.:')
          ParentFont = False
        end
        object Memo2: TfrxMemoView
          Align = baLeft
          Left = 117.165430000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataSet = frxDados
          DataSetName = 'frDados'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBDsuprimento."VALORSP"]')
          ParentFont = False
        end
      end
      object MasterData3: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 393.071120000000000000
        Width = 188.976500000000000000
        DataSet = frxDBdinheiro
        DataSetName = 'frxDBdinheiro'
        RowCount = 0
        object Memo3: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'DINHEIRO.+.:')
          ParentFont = False
        end
        object Memo4: TfrxMemoView
          Align = baLeft
          Left = 102.047310000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataField = 'VALOR'
          DataSet = frxDBdinheiro
          DataSetName = 'frxDBdinheiro'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBdinheiro."VALOR"]')
          ParentFont = False
        end
      end
      object MasterData4: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 291.023810000000000000
        Width = 188.976500000000000000
        DataSet = frxDBcartaod
        DataSetName = 'frxDBcartaod'
        RowCount = 0
        object Memo5: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'CARTAO DEB.+.:')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          Align = baLeft
          Left = 102.047310000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataField = 'VALOR'
          DataSet = frxDBcartaod
          DataSetName = 'frxDBcartaod'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBcartaod."VALOR"]')
          ParentFont = False
        end
      end
      object MasterData5: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 257.008040000000000000
        Width = 188.976500000000000000
        DataSet = frxDBcartaoc
        DataSetName = 'frxDBcartaoc'
        RowCount = 0
        object Memo9: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'CARTAO CRED.+.:')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Align = baLeft
          Left = 102.047310000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataField = 'VALOR'
          DataSet = frxDBcartaoc
          DataSetName = 'frxDBcartaoc'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBcartaoc."VALOR"]')
          ParentFont = False
        end
      end
      object MasterData6: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 222.992270000000000000
        Width = 188.976500000000000000
        DataSet = frxDBaprazo
        DataSetName = 'frxDBaprazo'
        RowCount = 0
        object Memo11: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'APRAZO.+.:')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Align = baLeft
          Left = 102.047310000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataField = 'VALOR'
          DataSet = frxDBaprazo
          DataSetName = 'frxDBaprazo'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBaprazo."VALOR"]')
          ParentFont = False
        end
        object Line2: TfrxLineView
          Left = -49.795300000000000000
          Width = 347.716760000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        Height = 268.346630000000000000
        Top = 536.693260000000000000
        Width = 188.976500000000000000
        object Memo13: TfrxMemoView
          Left = 22.102350000000000000
          Top = 12.338590000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            
              '+[<frxDBaprazo."VALOR">+<frxDBcartaoc."VALOR">+<frxDBcartaod."VA' +
              'LOR">+<frxDBdinheiro."VALOR">+<frxDBdep_transf."VALOR">+<frxDBpi' +
              'x."VALOR">+SUM(<frxDBDsuprimento."VALORSP">,MasterData2)]')
          ParentFont = False
        end
        object Memo14: TfrxMemoView
          Width = 185.196970000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Movimento Geral_Entrada..: +')
          ParentFont = False
        end
        object Memo15: TfrxMemoView
          Left = 22.440940000000000000
          Top = 44.236240000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            
              '+ [<frxDBdinheiro."VALOR">+SUM(<frxDBDsuprimento."VALORSP">,Mast' +
              'erData2)]')
          ParentFont = False
        end
        object Memo16: TfrxMemoView
          Top = 31.338590000000000000
          Width = 177.637910000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'TOTAL Dinheiro/Suprimento..: ')
          ParentFont = False
        end
        object Line3: TfrxLineView
          Left = -30.236240000000000000
          Top = 26.677180000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line4: TfrxLineView
          Left = -30.236240000000000000
          Top = 66.133890000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line5: TfrxLineView
          Left = -30.236240000000000000
          Top = 121.811070000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo20: TfrxMemoView
          Left = 0.779530000000000000
          Top = 94.149660000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'TOTAL SANGRIA..: ')
          ParentFont = False
        end
        object Memo21: TfrxMemoView
          Left = 26.267780000000000000
          Top = 134.826840000000000000
          Width = 147.401670000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            
              '[<frxDBdinheiro."VALOR">+SUM(<frxDBDsuprimento."VALORSP">,Master' +
              'Data2)-SUM(<frxDBDsangria."VALORS">,MasterData1)]')
          ParentFont = False
        end
        object Memo22: TfrxMemoView
          Left = 0.779530000000000000
          Top = 120.826840000000000000
          Width = 151.181200000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Saldo em Caixa...: ')
          ParentFont = False
        end
        object Line7: TfrxLineView
          Left = -29.456710000000000000
          Top = 151.740260000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo17: TfrxMemoView
          Left = -1.000000000000000000
          Top = 150.181200000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'Operador do caixa')
          ParentFont = False
        end
        object Line8: TfrxLineView
          Left = -26.456710000000000000
          Top = 177.637910000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo18: TfrxMemoView
          Left = 3.000000000000000000
          Top = 176.078850000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Gerente')
          ParentFont = False
        end
        object Memo23: TfrxMemoView
          Top = 204.094620000000000000
          Width = 188.976500000000000000
          Height = 64.252010000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Observa'#231#227'o:'
            '.'
            '.'
            '.'
            '.')
          ParentFont = False
        end
        object Line10: TfrxLineView
          Left = -55.779530000000000000
          Top = 200.315090000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line12: TfrxLineView
          Left = -76.897650000000000000
          Top = 0.220469999999977000
          Width = 476.220780000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo24: TfrxMemoView
          Top = 67.913420000000000000
          Width = 177.637910000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'TOTAL Suprimento..: ')
          ParentFont = False
        end
        object Line1: TfrxLineView
          Left = -30.236240000000000000
          Top = 94.708720000000000000
          Width = 411.968770000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object SysMemo1: TfrxSysMemoView
          Left = 60.252010000000000000
          Top = 78.811070000000000000
          Width = 113.385900000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '+[SUM(<frxDBDsuprimento."VALORSP">,MasterData2)]')
          ParentFont = False
        end
        object SysMemo2: TfrxSysMemoView
          Left = 65.149660000000000000
          Top = 105.047310000000000000
          Width = 109.606370000000000000
          Height = 15.118120000000000000
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '-[SUM(<frxDBDsangria."VALORS">,MasterData1)]')
          ParentFont = False
        end
      end
      object MasterData7: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 359.055350000000000000
        Width = 188.976500000000000000
        DataSet = frxDBdep_transf
        DataSetName = 'frxDBdep_transf'
        RowCount = 0
        object Memo27: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'DEP/TRANSF.+.:')
          ParentFont = False
        end
        object Memo28: TfrxMemoView
          Align = baLeft
          Left = 102.047310000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataField = 'VALOR'
          DataSet = frxDBdep_transf
          DataSetName = 'frxDBdep_transf'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBdep_transf."VALOR"]')
          ParentFont = False
        end
      end
      object MasterData8: TfrxMasterData
        FillType = ftBrush
        Height = 11.338590000000000000
        Top = 325.039580000000000000
        Width = 188.976500000000000000
        DataSet = frxDBpix
        DataSetName = 'frxDBpix'
        RowCount = 0
        object Memo30: TfrxMemoView
          Align = baLeft
          Width = 102.047310000000000000
          Height = 11.338580240000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'PIX.+.:')
          ParentFont = False
        end
        object Memo33: TfrxMemoView
          Align = baLeft
          Left = 102.047310000000000000
          Width = 86.929190000000000000
          Height = 11.338580240000000000
          DataField = 'VALOR'
          DataSet = frxDBpix
          DataSetName = 'frxDBpix'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDBpix."VALOR"]')
          ParentFont = False
        end
      end
    end
  end
  object qrcodcx: TUniQuery
    Connection = frmModulo.conexao
    SQL.Strings = (
      'SELECT MAX(DATA) AS DATA, MAX(COD_CAIXA) AS COD_CAIXA'
      'FROM CUPOM')
    Left = 184
    Top = 288
  end
  object frxcodcx: TfrxDBDataset
    UserName = 'frxcodcx'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'COD_CAIXA=COD_CAIXA')
    DataSet = qrcodcx
    BCDToCurrency = False
    Left = 224
    Top = 200
  end
  object frxDBDsuprimento1: TfrxDBDataset
    UserName = 'frxDBDsuprimento1'
    CloseDataSource = False
    DataSet = qrsuprimento1
    BCDToCurrency = False
    Left = 776
    Top = 408
  end
  object qrsuprimento1: TUniQuery
    Connection = frmModulo.conexao
    SQL.Strings = (
      'SELECT CODVENDEDOR, VALOR AS VALORS'
      'FROM NAO_FISCAL')
    Left = 805
    Top = 406
  end
  object qrsangria1: TUniQuery
    Connection = frmModulo.conexao
    SQL.Strings = (
      'SELECT CODVENDEDOR, VALOR AS VALORS'
      'FROM NAO_FISCAL')
    Left = 805
    Top = 454
  end
  object frxDBDsangria1: TfrxDBDataset
    UserName = 'frxDBDsangria1'
    CloseDataSource = False
    DataSet = qrsangria1
    BCDToCurrency = False
    Left = 776
    Top = 456
  end
  object frxDBdinheiro1: TfrxDBDataset
    UserName = 'frxDBdinheiro1'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'CODOPERADOR=CODOPERADOR'
      'NOME=NOME'
      'MOVIMENTO=MOVIMENTO'
      'VALOR=VALOR')
    DataSet = qrdinheiro1
    BCDToCurrency = False
    Left = 776
    Top = 120
  end
  object frxDBaprazo1: TfrxDBDataset
    UserName = 'frxDBaprazo1'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'CODOPERADOR=CODOPERADOR'
      'NOME=NOME'
      'MOVIMENTO=MOVIMENTO'
      'VALOR=VALOR')
    DataSet = qraprazo1
    BCDToCurrency = False
    Left = 776
    Top = 168
  end
  object qrdinheiro1: TUniQuery
    Connection = frmModulo.conexao
    SQL.Strings = (
      
        'select CX.DATA, CX.codoperador, FUN.nome, CX.movimento, SUM(CX.V' +
        'ALOR) AS VALOR'
      'FROM C000044 CX, c000008 FUN'
      'WHERE CX.movimento = 3'
      'group BY CX.data, CX.movimento, CX.codoperador, FUN.nome')
    Left = 805
    Top = 118
  end
  object qraprazo1: TUniQuery
    Connection = frmModulo.conexao
    SQL.Strings = (
      
        'select CX.DATA, CX.codoperador, FUN.nome, CX.movimento, SUM(CX.V' +
        'ALOR) AS VALOR'
      'FROM C000044 CX, c000008 FUN'
      'WHERE CX.movimento = 4'
      'group BY CX.data, CX.movimento, CX.codoperador, FUN.nome')
    Left = 805
    Top = 166
  end
  object frxDBcartaoc1: TfrxDBDataset
    UserName = 'frxDBcartaoc1'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'CODOPERADOR=CODOPERADOR'
      'NOME=NOME'
      'MOVIMENTO=MOVIMENTO'
      'VALOR=VALOR')
    DataSet = qrcartaoc1
    BCDToCurrency = False
    Left = 776
    Top = 216
  end
  object frxDBcartaod1: TfrxDBDataset
    UserName = 'frxDBcartaod1'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'CODOPERADOR=CODOPERADOR'
      'NOME=NOME'
      'MOVIMENTO=MOVIMENTO'
      'VALOR=VALOR')
    DataSet = qrcartaod1
    BCDToCurrency = False
    Left = 776
    Top = 264
  end
  object qrcartaoc1: TUniQuery
    Connection = frmModulo.conexao
    SQL.Strings = (
      
        'select CX.DATA, CX.codoperador, FUN.nome, CX.movimento, SUM(CX.V' +
        'ALOR) AS VALOR'
      'FROM C000044 CX, c000008 FUN'
      'WHERE CX.movimento = 7'
      'group BY CX.data, CX.movimento, CX.codoperador, FUN.nome')
    Left = 805
    Top = 214
  end
  object qrcartaod1: TUniQuery
    Connection = frmModulo.conexao
    SQL.Strings = (
      
        'select CX.DATA, CX.codoperador, FUN.nome, CX.movimento, SUM(CX.V' +
        'ALOR) AS VALOR'
      'FROM C000044 CX, c000008 FUN'
      'WHERE CX.movimento = 8'
      'group BY CX.data, CX.movimento, CX.codoperador, FUN.nome')
    Left = 805
    Top = 262
  end
  object frxDBpix1: TfrxDBDataset
    UserName = 'frxDBpix1'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'CODOPERADOR=CODOPERADOR'
      'NOME=NOME'
      'MOVIMENTO=MOVIMENTO'
      'VALOR=VALOR')
    DataSet = qrpix1
    BCDToCurrency = False
    Left = 776
    Top = 312
  end
  object frxDBdep_transf1: TfrxDBDataset
    UserName = 'frxDBdep_transf1'
    CloseDataSource = False
    FieldAliases.Strings = (
      'DATA=DATA'
      'CODOPERADOR=CODOPERADOR'
      'NOME=NOME'
      'MOVIMENTO=MOVIMENTO'
      'VALOR=VALOR')
    DataSet = qrdep_transf1
    BCDToCurrency = False
    Left = 776
    Top = 360
  end
  object qrpix1: TUniQuery
    Connection = frmModulo.conexao
    SQL.Strings = (
      
        'select CX.DATA, CX.codoperador, FUN.nome, CX.movimento, SUM(CX.V' +
        'ALOR) AS VALOR'
      'FROM C000044 CX, c000008 FUN'
      'WHERE CX.movimento = 5'
      'group BY CX.data, CX.movimento, CX.codoperador, FUN.nome')
    Left = 805
    Top = 310
  end
  object qrdep_transf1: TUniQuery
    Connection = frmModulo.conexao
    SQL.Strings = (
      
        'select CX.DATA, CX.codoperador, FUN.nome, CX.movimento, SUM(CX.V' +
        'ALOR) AS VALOR'
      'FROM C000044 CX, c000008 FUN'
      'WHERE CX.movimento = 6'
      'group BY CX.data, CX.movimento, CX.codoperador, FUN.nome')
    Left = 805
    Top = 358
  end
end
