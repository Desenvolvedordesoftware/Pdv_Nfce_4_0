...
uses
  ...
  ZAbstractRODataset, ZAbstractDataset, ZDataset, AdvGlowButton, MemDS,
  DBAccess, Uni;
...
begin
    ...
    if loc.Text <= '' then begin
        Application.MessageBox('Favor informar DESCRIÇÃO!','Erro',mb_ok+mb_iconerror);
        exit;
      end;


             qrobs_orcamento.Close;
             qrobs_orcamento.sql.clear;
             qrobs_orcamento.sql.add('update cupom_item_temp set PEGO_PORQUEM = :OBS ');
             qrobs_orcamento.Parambyname('OBS').asstring := loc.Text;
             qrobs_orcamento.ExecSQL;
             close;
...
procedure Tfrmxloc_modelo.GravarClick(Sender: TObject);
begin
             qrobs_orcamento.Close;
             qrobs_orcamento.sql.clear;
             qrobs_orcamento.sql.add('update cupom_item_temp set PEGO_PORQUEM = :OBS ');
